...
BOOST_AUTO_TEST_CASE(Alias) {
    ...
    boost::shared_ptr<QuerySession> qs = prepareTestQuerySession(qsTest, sql, expected_err_msg);
...
BOOST_AUTO_TEST_CASE(Simple) {
    std::string sql = "select pm_declErr, chunkId, ra_Test from LSST.Object where bMagF > 20.0 GROUP BY chunkId;";
     ...
     boost::shared_ptr<QuerySession> qs = prepareTestQuerySession(qsTest, sql);
     boost::shared_ptr<QueryContext> context = qs->dbgGetContext();
     BOOST_CHECK(context);
}
