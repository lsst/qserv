...
/// &&& Change this to accept a vector of util::Command::Ptr const&
void BlendScheduler::queCmd(std::vector<util::Command::Ptr> const& cmds) {
    ...
    for (auto const& cmd : cmds) {
        ...
        if (task == nullptr) {
            LOGS(_log, LOG_LVL_WARN, "&&& BlendScheduler::queCmd c");
        ...
        LOGS(_log, LOG_LVL_WARN, "&&& BlendScheduler::queCmd d");
        if (task->msg == nullptr) {
        ...
        LOGS(_log, LOG_LVL_WARN, "&&& BlendScheduler::queCmd e");
        ...
        if (first) {
            LOGS(_log, LOG_LVL_WARN, "&&& BlendScheduler::queCmd f");
            first = false;
            ...
            if (scanTables.size() <= 0 || interactive) {
                LOGS(_log, LOG_LVL_WARN, "&&& BlendScheduler::queCmd g");
                ...
                LOGS(_log, LOG_LVL_WARN, "&&& BlendScheduler::queCmd i");
                {   // Find the scheduler responsible for this 'scanPriority'.
                    lock_guard<mutex> lg(_schedMtx);
                    LOGS(_log, LOG_LVL_WARN, "&&& BlendScheduler::queCmd j");
                    for (auto const& sched : _schedulers) {
                        LOGS(_log, LOG_LVL_WARN, "&&& BlendScheduler::queCmd j1");
                ...
                LOGS(_log, LOG_LVL_WARN, "&&& BlendScheduler::queCmd k");
