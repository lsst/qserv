...
/**
  ...
  */
class ConfigurationMap: public ConfigurationBase {
public:
     ...
     * @throw std::runtime_error Ff the input configuration is not consistent with expectations
...
private:
    ...
    void _set(T& var, T val, bool allowZero=false) {
        ...
        if (not allowZero and val == 0) {
            throw std::invalid_argument(
                    "ConfigurationMap::" + std::string(__func__) + "<numeric>  0 value is not allowed");
        }
        var = val;
    }
    ...
    void _set(std::string& var, std::string const& val, bool allowEmpty=false) {
        ...
        if (not allowEmpty and val.empty()) {
            throw std::invalid_argument(
                    "ConfigurationMap::" + std::string(__func__) + "<string>  empty value is not allowed");
        }
        var = val;
    }
