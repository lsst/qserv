...
ConfigurationMap::ConfigurationMap(map<string, string> const& kvMap)
    :   ConfigurationBase(),
        _log(LOG_GET("lsst.qserv.replica.ConfigurationMap")) {

    _loadConfiguration(util::ConfigStore(kvMap));
}


string ConfigurationMap::_classMethodContext(string const& func) {
    return "ConfigurationMap::" + func;
}


void ConfigurationMap::addWorker(WorkerInfo const& info) {
...
void ConfigurationMap::deleteWorker(string const& name) {
...
WorkerInfo ConfigurationMap::disableWorker(string const& name,
                                             bool disable,
...
WorkerInfo ConfigurationMap::setWorkerReadOnly(string const& name,
                                                 bool readOnly,
                                                 bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name
...
WorkerInfo ConfigurationMap::setWorkerSvcHost(string const& name,
                                                string const& host,
                                                bool updatePersistentState) {
...
WorkerInfo ConfigurationMap::setWorkerSvcPort(string const& name,
                                                uint16_t port,
                                                bool updatePersistentState) {
...
WorkerInfo ConfigurationMap::setWorkerFsHost(string const& name,
                                               string const& host,
                                               bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " host=" << host);
...
WorkerInfo ConfigurationMap::setWorkerFsPort(string const& name,
                                               uint16_t port,
                                               bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " port=" << port);
...
WorkerInfo ConfigurationMap::setWorkerDataDir(string const& name,
...
WorkerInfo ConfigurationMap::setWorkerDbHost(std::string const& name,
...
WorkerInfo ConfigurationMap::setWorkerDbPort(std::string const& name,
                                               uint16_t port,
                                               bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " port=" << port);
...
WorkerInfo ConfigurationMap::setWorkerDbUser(std::string const& name,
...
WorkerInfo ConfigurationMap::setWorkerLoaderHost(string const& name,
                                                   string const& host,
                                                   bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " host=" << host);
...
WorkerInfo ConfigurationMap::setWorkerLoaderPort(string const& name,
                                                   uint16_t port,
                                                   bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " port=" << port);
...
WorkerInfo ConfigurationMap::setWorkerLoaderTmpDir(string const& name,
                                                     string const& tmpDir,
                                                     bool updatePersistentState) {
...
WorkerInfo ConfigurationMap::setWorkerExporterHost(string const& name,
                                                     string const& host,
                                                     bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " host=" << host);
...
WorkerInfo ConfigurationMap::setWorkerExporterPort(string const& name,
                                                     uint16_t port,
                                                     bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " port=" << port);
...
WorkerInfo ConfigurationMap::setWorkerExporterTmpDir(string const& name,
                                                       string const& tmpDir,
                                                       bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " tmpDir=" << tmpDir);
...
WorkerInfo ConfigurationMap::setWorkerHttpLoaderHost(string const& name,
                                                       string const& host,
                                                       bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " host=" << host);
    auto itr = safeFindWorker(name, _classMethodContext(__func__));
...
WorkerInfo ConfigurationMap::setWorkerHttpLoaderPort(string const& name,
                                                       uint16_t port,
                                                       bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " port=" << port);
    auto itr = safeFindWorker(name, _classMethodContext(__func__));
...
WorkerInfo ConfigurationMap::setWorkerHttpLoaderTmpDir(string const& name,
                                                         string const& tmpDir,
                                                         bool updatePersistentState) {
    LOGS(_log, LOG_LVL_DEBUG, context(__func__) << "  name=" << name << " tmpDir=" << tmpDir);
    auto itr = safeFindWorker(name, _classMethodContext(__func__));
...
DatabaseFamilyInfo ConfigurationMap::addDatabaseFamily(DatabaseFamilyInfo const& info) {
...
void ConfigurationMap::deleteDatabaseFamily(string const& name) {
...
DatabaseInfo ConfigurationMap::addDatabase(DatabaseInfo const& info) {
...
DatabaseInfo ConfigurationMap::publishDatabase(string const& name) {
...
DatabaseInfo ConfigurationMap::unPublishDatabase(string const& name) {
...
void ConfigurationMap::deleteDatabase(string const& name) {
...
DatabaseInfo ConfigurationMap::addTable(
...
DatabaseInfo ConfigurationMap::deleteTable(string const& database,
                                             string const& table) {
...
void ConfigurationMap::_loadConfiguration(util::ConfigStore const& configStore) {
    ...
    uint16_t commonWorkerHttpLoaderPort;
    string   commonWorkerHttpLoaderTmpDir;
    ...
    ::parseKeyVal(configStore, "worker.http_loader_port",    commonWorkerHttpLoaderPort,   defaultWorkerHttpLoaderPort);
    ::parseKeyVal(configStore, "worker.http_loader_tmp_dir", commonWorkerHttpLoaderTmpDir, defaultWorkerHttpLoaderTmpDir);
    ...
    for (string const& name: workers) {
        ...
        ::parseKeyVal(configStore, section+".http_loader_host",    workerInfo.httpLoaderHost,   defaultWorkerHttpLoaderHost);
        ::parseKeyVal(configStore, section+".http_loader_port",    workerInfo.httpLoaderPort,   commonWorkerHttpLoaderPort);
        ::parseKeyVal(configStore, section+".http_loader_tmp_dir", workerInfo.httpLoaderTmpDir, commonWorkerHttpLoaderTmpDir);
        ...
        ConfigurationBase::translateWorkerDir(workerInfo.httpLoaderTmpDir, name);
    }
