...
void IngestClient::send() {
    ...
    unique_ptr<char*> record = new char[_recordSizeBytes];
    bool eof = false;
    do {
        ProtocolIngestData data;
        if (file.read(*record, _recordSizeBytes)) {
            size_t const num = file.gcount();
            data.set_data(*record, num);
            _sizeBytes += num;
        } else {
            eof = true;
            break;
...
void IngestClient::_connectImpl() {
    ...
    request.set_fields_terminated_by(fieldsTerminatedBy);
    request.set_fields_enclosed_by(fieldsEnclosedBy);
    request.set_fields_escaped_by(fieldsEscapedBy);
    request.set_lines_terminated_by(set_linesTerminatedBy);
    _bufferPtr->resize();
