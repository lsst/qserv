...
json general2json(Configuration::Ptr const& config) {
    json result;
    for (auto&& itr: ConfigurationSchema::parameters()) {
        string const& category = itr.first;
        for (auto&& parameter: itr.second) {
            result.push_back({
                {"updatable", ConfigurationSchema::readOnly(category, parameter) ? 1 : 0},
                {"category", category},
                {"parameter", parameter},
                {"description", ConfigurationSchema::description(category, parameter)},
                {"value", ConfigurationSchema::securityContext(category, parameter) ?
                        "xxxxxx" : config->getAsString(category, parameter)}
            });
        }
    }
    return result;
...
json HttpConfigurationModule::_get() {
    ...
    result["config"]["general"] = general2json(config);
    return result;
}
...
json HttpConfigurationModule::_updateGeneral() {
    ...
    if (ConfigurationSchema::readOnly(category, parameter)) {
        throw invalid_argument(context() + "::" + string(__func__) +
                "  this is the read-only parametr that can't be changed via this method.");
    }
...
json HttpConfigurationModule::_updateWorker() {
    ...
    // Amend requested worker attribute changes into the latest transient state
    ...
    auto const amendBool = [&](string const& func, string const& name, bool& out) {
        int const val = query().optionalInt(name);
        debug(func, name + "=" + to_string(val));
        WorkerInfo::amend(val, out);
    };
    auto const amendString = [&](string const& func, string const& name, string& out) {
        string const val = query().optionalString(name);
        debug(func, name + "=" + val);
        WorkerInfo::amend(val, out);
    };
    auto const amendPort = [&](string const& func, string const& name, uint16_t& out) {
        uint16_t const val = query().optionalUInt16(name);
        debug(func, name + "=" + to_string(val));
        WorkerInfo::amend(val, out);
    };

    amendBool(__func__, "is_enabled",   info.isEnabled);
    amendBool(__func__, "is_read_only", info.isReadOnly);

    amendString(__func__, "svc_host", info.svcHost);
    amendPort(  __func__, "svc_port", info.svcPort);

    amendString(__func__, "fs_host",  info.fsHost);
    amendPort(  __func__, "fs_port",  info.fsPort);
    amendString(__func__, "data_dir", info.dataDir);

    amendString(__func__, "db_host", info.dbHost);
    amendPort(  __func__, "db_port", info.dbPort);
    amendString(__func__, "db_user", info.dbUser);

    amendString(__func__, "loader_host",    info.loaderHost);
    amendPort(  __func__, "loader_port",    info.loaderPort);
    amendString(__func__, "loader_tmp_dir", info.loaderTmpDir);

    amendString(__func__, "exporter_host",    info.exporterHost);
    amendPort(  __func__, "exporter_port",    info.exporterPort);
    amendString(__func__, "exporter_tmp_dir", info.exporterTmpDir);

    amendString(__func__, "http_loader_host",    info.httpLoaderHost);
    amendPort(  __func__, "http_loader_port",    info.httpLoaderPort);
    amendString(__func__, "http_loader_tmp_dir", info.httpLoaderTmpDir);
...
json HttpConfigurationModule::_addDatabase() {
    ...
    DatabaseInfo info;
    info.name   = body().required<string>("database");
    info.family = body().required<string>("family");

    debug(__func__, "database=" + info.name);
    debug(__func__, "family="   + info.family);

    json result;
    result["config"]["databases"][info.name] =
            controller()->serviceProvider()->config()->addDatabase(info).toJson();
    return result;
}
