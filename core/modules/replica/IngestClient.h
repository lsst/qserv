...
public:
    ...
    static Ptr connect(std::string const& workerHost,
                       ...
                       std::string const& fieldsEnclosedBy=std::string(),
                       std::string const& fieldsEscapedBy=std::string(),
                       std::string const& linesTerminatedBy=std::string(),
                       size_t recordSizeBytes=1048576);
...
private:
    ...
    /// The maximum number of bytes to be read from an input file
    /// in one record. Th erecord will be sent to the server w/o
    /// any modifications.
    size_t const _recordSizeBytes = 1024 * 1024;
