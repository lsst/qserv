...
BOOST_AUTO_TEST_CASE(ConfigurationTest) {
    ...
    BOOST_REQUIRE_NO_THROW(config = Configuration::load(ConfigurationSchema::testConfigData()));
    ...
    map<string, set<string>> const generalCategoriesAndParameters = {
        {   "meta",
            {   "version"
            }
        },
    ...
    BOOST_CHECK(config->parameters() == generalCategoriesAndParameters);
    ...
    BOOST_CHECK(production.overlap == 0.01667);
    ...
    BOOST_CHECK(test.overlap == 0.001);
    ...
    BOOST_CHECK(newFamilyAdded.overlap == 0.001);
    ...
    // Adding new databases.
    {
        ...
        BOOST_REQUIRE_NO_THROW(newInfo = config->addDatabase(info));
        ...
        BOOST_CHECK_THROW(config->addDatabase(newInfo), std::invalid_argument);
    }
    {
        DatabaseInfo info;
        info.name = "";
        BOOST_CHECK_THROW(config->addDatabase(info), std::invalid_argument);
    ...
    BOOST_REQUIRE_NO_THROW(addedWorkerE = config->addWorker(workerF));
    BOOST_CHECK(addedWorkerE.name == workerF.name);
    BOOST_CHECK(addedWorkerE.isEnabled == workerF.isEnabled);
    BOOST_CHECK(addedWorkerE.isReadOnly == workerF.isReadOnly);
    BOOST_CHECK(addedWorkerE.svcHost == workerF.svcHost);
    BOOST_CHECK(addedWorkerE.svcPort == config->get<uint16_t>("worker_defaults", "svc_port"));
    BOOST_CHECK(addedWorkerE.fsHost == workerF.svcHost);
    BOOST_CHECK(addedWorkerE.fsPort == config->get<uint16_t>("worker_defaults", "fs_port"));
    BOOST_CHECK(addedWorkerE.dataDir == config->get<string>("worker_defaults", "data_dir"));
    BOOST_CHECK(addedWorkerE.dbHost == workerF.svcHost);
    BOOST_CHECK(addedWorkerE.dbPort == config->get<uint16_t>("worker_defaults", "db_port"));
    BOOST_CHECK(addedWorkerE.dbUser == config->get<string>("worker_defaults", "db_user"));
    BOOST_CHECK(addedWorkerE.loaderHost == workerF.svcHost);
    BOOST_CHECK(addedWorkerE.loaderPort == config->get<uint16_t>("worker_defaults", "loader_port"));
    BOOST_CHECK(addedWorkerE.loaderTmpDir == config->get<string>("worker_defaults", "loader_tmp_dir"));
    BOOST_CHECK(addedWorkerE.exporterHost == workerF.svcHost);
    BOOST_CHECK(addedWorkerE.exporterPort == config->get<uint16_t>("worker_defaults", "exporter_port"));
    BOOST_CHECK(addedWorkerE.exporterTmpDir == config->get<string>("worker_defaults", "exporter_tmp_dir"));
    BOOST_CHECK(addedWorkerE.httpLoaderHost == workerF.svcHost);
    BOOST_CHECK(addedWorkerE.httpLoaderPort == config->get<uint16_t>("worker_defaults", "http_loader_port"));
    BOOST_CHECK(addedWorkerE.httpLoaderTmpDir == config->get<string>("worker_defaults", "http_loader_tmp_dir"));
