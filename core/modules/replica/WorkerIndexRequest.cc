...
bool WorkerIndexRequest::execute() {
    ...
    try {
        ...
        if (ec.value() != 0) {
            ...
            setStatus(lock, STATUS_FAILED, ProtocolStatusExt::FOLDER_CREATE);
        }
        ...
        if (ec.value() != 0) {
            ...
            LOGS(_log, LOG_LVL_ERROR, context(__func__) << "  " << _error);
            setStatus(lock, STATUS_FAILED, ProtocolStatusExt::FILE_CREATE);
        }
        ...
        else                 setStatus(lock, STATUS_FAILED, ProtocolStatusExt::FILE_READ);

    } catch(database::mysql::ER_NO_SUCH_TABLE_ const& ex) {
        LOGS(_log, LOG_LVL_ERROR, context(__func__) << "  MySQL error: " << ex.what());
        _error = ex.what();
        setStatus(lock, STATUS_FAILED, ProtocolStatusExt::NO_SUCH_TABLE);
    } catch(database::mysql::ER_PARTITION_MGMT_ON_NONPARTITIONED_ const& ex) {
        LOGS(_log, LOG_LVL_ERROR, context(__func__) << "  MySQL error: " << ex.what());
        _error = ex.what();
        setStatus(lock, STATUS_FAILED, ProtocolStatusExt::NOT_PARTITIONED_TABLE);
    } catch(database::mysql::ER_UNKNOWN_PARTITION_ const& ex) {
        LOGS(_log, LOG_LVL_ERROR, context(__func__) << "  MySQL error: " << ex.what());
        _error = ex.what();
        setStatus(lock, STATUS_FAILED, ProtocolStatusExt::NO_SUCH_PARTITION);
    } catch(database::mysql::Error const& ex) {
        LOGS(_log, LOG_LVL_ERROR, context(__func__) << "  MySQL error: " << ex.what());
        _error = ex.what();
        setStatus(lock, STATUS_FAILED, ProtocolStatusExt::MYSQL_ERROR);
    } catch (invalid_argument const& ex) {
        LOGS(_log, LOG_LVL_ERROR, context(__func__) << "  exception: " << ex.what());
        _error = ex.what();
        setStatus(lock, STATUS_FAILED, ProtocolStatusExt::INVALID_PARAM);
    } catch (out_of_range const& ex) {
        LOGS(_log, LOG_LVL_ERROR, context(__func__) << "  exception: " << ex.what());
        _error = ex.what();
        setStatus(lock, STATUS_FAILED, ProtocolStatusExt::LARGE_RESULT);
