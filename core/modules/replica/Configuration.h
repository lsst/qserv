...
template <>
struct TypeConversionTrait<bool> {
    static std::string to_string(bool const val) { return val ? "1" : "0"; }
};
}   // namespace detail
...
public:
     ...
     *   conditions are met. 
     */
    static void amend(int const in, bool& out) {
        if ((in == 0) && out) {
            out = false;
        } else if ((in > 0) && !out) {
            out = true;
     ...
     *   conditions are met. 
     */
    static void amend(std::string const& in, std::string& out) {
        if (!in.empty() && (out != in)) out = in;
    }

    /**
     * This function is specialized for 16-bit port numbers. It treats
     * any noon-zero input as an explicit user input that needs to be checked
     ...
     *   conditions are met. 
     */
    static void amend(uint16_t const in, uint16_t& out) {
        if ((in != 0) && (out != in)) out = in;
    }

    /**
     * Construct from another worker descriptor using a required collection of
     * the default parameters to compesate for incomplete info in the input descriptor.
     * @param defaults The required JSON object to be used of a source of the default
     ...
     * @param defaults The optional object to be used of a source of the default values
...
public:
    ...
    /**
     * Construct from a JSON object.
     * @note Passing an empty JSON object or 'nullptr' as a value of the optional
     *   parameter 'families' will disable teh optional step of the family validation.
     *   This is safe to do once if he object is pulled from the transient state
     ...
     *   for validating database definition.
     * @throw std::invalid_argument If the input object can't be parsed, or if it has
     *   incorrect schema.
     */
    explicit DatabaseInfo(nlohmann::json const& obj=nlohmann::json::object(),
                          nlohmann::json const& families=nlohmann::json());
 ...
 *   store as well. The default value is set to 'tru' since this would be a desired
...
public:
     ...
     * @note The parameter is allowed to not exist.
     * @param category The name of the parameter's category.
     * @param param The name of the parameter within its category.
    ...
    void set(std::string const& category, std::string const& param, T const& val, bool updatePersistentState=true) {
        ...
        // Validate the input for presence of the empty strings and zero initialized numeric
        // values unless those are permitted by the schema.
        T const& validated = ConfigurationSchema::emptyAllowed(category, param) ? val :
                detail::TypeConversionTrait<T>::validate(context_, val);
        try {
            nlohmann::json& obj = _get(lock, category, param);
            if (updatePersistentState && (_connectionPtr != nullptr)) {
                // Convert the value into a string. Tis is possible because values of
                // the general parameters are stored as strings in the corresponding MySQL table.
                _set(category, param,  detail::TypeConversionTrait<T>::to_string(validated));
            }
            obj = validated;
    ...
    size_t requestBufferSizeBytes() const;
    void setRequestBufferSizeBytes(size_t val);

    unsigned int retryTimeoutSec() const;
    void setRetryTimeoutSec(unsigned int val);

    size_t controllerThreads() const;
    void setControllerThreads(size_t val);

    uint16_t controllerHttpPort() const;
    void setControllerHttpPort(uint16_t val);

    size_t controllerHttpThreads() const;
    void setControllerHttpThreads(size_t val);

    unsigned int controllerRequestTimeoutSec() const;
    void setControllerRequestTimeoutSec(unsigned int val, bool updatePersistentState=true);

    std::string controllerEmptyChunksDir() const;

    unsigned int jobTimeoutSec() const;
    void setJobTimeoutSec(unsigned int val);

    unsigned int jobHeartbeatTimeoutSec() const;
    void setJobHeartbeatTimeoutSec(unsigned int val);

    // Parameters driving Replication/Ingest system's integration with Qserv.

    bool xrootdAutoNotify() const;
    void setXrootdAutoNotify(bool val);

    std::string xrootdHost() const;
    void setXrootdHost(std::string const& val);

    uint16_t xrootdPort() const;
    void setXrootdPort(uint16_t val);

    unsigned int xrootdTimeoutSec() const;
    void setXrootdTimeoutSec(unsigned int val, bool updatePersistentState=true);

    // Read-only parameters of the Database Service of the system. These parameters
    // are deduced from the 'configUrl' to point to the same MySQL server where
    // the initial configuration of teh system is loaded.
    ...
    // Parameters of the Database Service's connection pool.

    size_t databaseServicesPoolSize() const;
    void setDatabaseServicesPoolSize(size_t val);

    // Parameters for accessing MySQL services of Qserv 'czar'.

    std::string qservMasterDatabaseHost() const;
    void setQservMasterDatabaseHost(std::string const& val);

    uint16_t qservMasterDatabasePort() const;
    void setQservMasterDatabasePort(uint16_t val);

    std::string qservMasterDatabaseUser() const;
    void setQservMasterDatabaseUser(std::string const& val);

    std::string qservMasterDatabaseName() const;
    void setQservMasterDatabaseName(std::string const& val);

    size_t qservMasterDatabaseServicesPoolSize() const;
    void setQservMasterDatabaseServicesPoolSize(size_t val);

    std::string qservMasterDatabaseTmpDir() const;
    void setQservMasterDatabaseTmpDir(std::string const& val);
     ...
     */
    DatabaseInfo addDatabase(DatabaseInfo const& info);
    ...
    std::string workerTechnology() const;
    void setWorkerTechnology(std::string const& val);

    size_t workerNumProcessingThreads() const;
    void setWorkerNumProcessingThreads(size_t val);

    size_t fsNumProcessingThreads() const;
    void setFsNumProcessingThreads(size_t val);

    size_t workerFsBufferSizeBytes() const;
    void setWorkerFsBufferSizeBytes(size_t val);

    size_t loaderNumProcessingThreads() const;
    void setLoaderNumProcessingThreads(size_t val);

    size_t exporterNumProcessingThreads() const;
    void setExporterNumProcessingThreads(size_t val);

    size_t httpLoaderNumProcessingThreads() const;
    void setHttpLoaderNumProcessingThreads(size_t val);

    // This section has methods defining default values that are used when incomplete
    // worker definitions are provided by a user when registering/updating workers, or
    // or when reading worker descriptions from the persistent state. The incomplete
    // definitions are allowed since each worker of a setup would normally have the same
    // value of each parameter defined in this section. This approach also allows more
    // flexibility in configuring workers should this be needed in specific cases.
    //
    // The methods would throw the same exceptions as explained in other similar
    // single parameter "getter" or "setter" methods in the class's contract.

    uint16_t workerDefaultSvcPort() const;
    void setWorkerDefaultSvcPort(uint16_t val);

    uint16_t workerDefaultFsPort() const;
    void setWorkerDefaultFsPort(uint16_t val);

    std::string workerDefaultDataDir() const;
    void setWorkerDefaultDataDir(std::string const& val);

    uint16_t workerDefaultDbPort() const;
    void setWorkerDefaultDbPort(uint16_t val);

    std::string workerDefaultDbUser() const;
    void setWorkerDefaultDbUser(std::string const& val);

    uint16_t workerDefaultLoaderPort() const;
    void setWorkerDefaultLoaderPort(uint16_t val);

    std::string workerDefaultLoaderTmpDir() const;
    void setWorkerDefaultLoaderTmpDir(std::string const& val);

    uint16_t workerDefaultExporterPort() const;
    void setWorkerDefaultExporterPort(uint16_t val);

    std::string workerDefaultExporterTmpDir() const;
    void setWorkerDefaultExporterTmpDir(std::string const& val);

    uint16_t workerDefaultHttpLoaderPort() const;
    void setWorkerDefaultHttpLoaderPort(uint16_t val);

    std::string workerDefaultHttpLoaderTmpDir() const;
    void setWorkerDefaultHttpLoaderTmpDir(std::string const& val);
...
private:
     ...
     * @param obj An object to be used as a source for amending th edefault values
     ...
     * @note This methgod won't check for correctness of the parameter values. It's expected
    ...
    /**
     * Database family validator & locators.
     * 
     * @param lock The lock on '_mtx' to be acquired prior to calling the method.
     * @param name The name of a family to be located.
     * @param mustExist The entry must exist if 'true', or not if 'false'.
     * @throws std::invalid_argument If the name is empty or if the entry's state is not
     *   what's required in a value of the parameter 'exist'.
     */
    void      _assertDatabaseFamilyExists(util::Lock const &lock, std::string const& name, bool mustExist) const;
    nlohmann::json const& _databaseFamily(util::Lock const &lock, std::string const& name) const;
    nlohmann::json&       _databaseFamily(util::Lock const &lock, std::string const& name);

    /**
     * Database validator & locators.
     * @param lock The lock on '_mtx' to be acquired prior to calling the method.
     * @param name The name of a database to be located.
     * @param mustExist The entry must exist if 'true', or not if 'false'.
     * @throws std::invalid_argument If the name is empty or if the entry's state is not
     *   what's required in a value of the parameter 'exist'.
     */
    void      _assertDatabaseExists(util::Lock const& lock, std::string const& name, bool mustExist) const;
    nlohmann::json const& _database(util::Lock const& lock, std::string const& name) const;
    nlohmann::json&       _database(util::Lock const& lock, std::string const& name);

    /**
     * Validate the input and locate a database entry.
     * @param lock The lock on '_mtx' to be acquired prior to calling the method.
     * @param name The name of a database to be located.
     * @throws std::invalid_argument If the database name is empty or if the database
     *   doesn't exist.
     */
     ...
     * @param lock The lock on '_mtx' to be acquired prior to calling the method.
     * @param obj The transient database objects to be updated.
     ...
     * Database validator.
     * @param lock The lock on '_mtx' to be acquired prior to calling the method.
     * @param databaseJson A JSON object representing a database.
     * @param table The name of a table to be located.
     * @param mustExist The entry must exist if 'true', or not if 'false'.
     * @throws std::invalid_argument If the name is empty or if the entry's state is not
     *   what's required in a value of the parameter 'exist'.
     */
    void _assertTableExists(util::Lock const& lock,
                            nlohmann::json const& databaseJson,
                            std::string const& table,
     ...
     * @throw std::invalid_argument If any problems were found.
     */
    void _validateTableParameters(util::Lock const& lock,
    ...
    /**
     * Worker validator & locators.
     * @param lock The lock on '_mtx' to be acquired prior to calling the method.
     * @param name The name of a worker to be located.
     * @param mustExist The entry must exist if 'true', or not if 'false'.
     * @throws std::invalid_argument If the name is empty or if the entry's state is not
     *   what's required in a value of the parameter 'exist'.
     */
    void      _assertWorkerExists(util::Lock const& lock, std::string const& name, bool mustExist) const;
    nlohmann::json const& _worker(util::Lock const& lock, std::string const& name) const;
    nlohmann::json&       _worker(util::Lock const& lock, std::string const& name);
