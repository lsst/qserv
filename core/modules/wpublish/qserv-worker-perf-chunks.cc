...
public:
    ...
    static Ptr create(unsigned int maxRequestsAllowed) {
        return Ptr(new Counter(maxRequestsAllowed));
    }
...
public:
    ...
    static Ptr create(Counter::Ptr const& numRequestsInFlight,
                      XrdSsiService* serviceProvider,
                      string const& resourcePath,
                      string const& payload,
                      OnFinish const& onFinish,
                      bool memoryCleanup) {
        return Ptr(new RequestManager(
            numRequestsInFlight, serviceProvider, resourcePath, payload, onFinish, memoryCleanup
        ));
    }
...
private:
    void _finished(RequestT::Status status,
                   string const& error,
                   string const& sent,
    ...
    RequestManager(Counter::Ptr const& numRequestsInFlight,
                   XrdSsiService* serviceProvider,
                   string const& resourcePath,
                   string const& payload,
                   OnFinish const& onFinish,
...
int test() {
    ...
    for (unsigned int tIdx = 0; tIdx < numThreads; ++tIdx) {
        ...
        threads.emplace_back(new thread(
            [&numRequestsInFlight,&payload,memoryCleanup](
                    vector<string> const& jobs,
                    XrdSsiService* serviceProvider) {
...
int main(int argc, const char* const argv[]) {
    ...
    try {
        util::CmdLineParser parser(
            ...
            "\n"
            "Flags and options:\n"
            "  --service=<provider>      - A location of a service provider (default: 'localhost:1094').\n"
            ...
            "  --value=<value>           - An arbitrary string to be used in composing messages sent\n"
            "                              to the services (default: 'a').\n"
