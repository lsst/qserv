...
protected:
    ...
    DbITransaction(sql::SqlConnection& conn) : sql::SqlTransactionScope(conn) {}
};
...
set<int> DbInterfaceMySql::getEmptyChunks(string const& dbName) {
    string funcName("getEmptyChunksFromDb");
    lock_guard<mutex> sync(_dbMutex);

    DbITransaction::Ptr trans = sql::SqlTransactionScope::create<DbITransaction>(_conn);

    /// This must match the name of the table generated by
    /// dataloader.py DataLoader _makeEmptyChunksTable()
    string emptyChunkTbl = getEmptyChunksTableName(dbName);
