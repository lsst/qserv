...
"""
#
# This file is borrowed from https://github.com/miguelgrinberg/Flask-HTTPAuth
...
class HTTPDigestAuth(HTTPAuth):
    ...
    def authenticate_header(self):
        auth_nonce = self.get_nonce()
        auth_opaque = self.get_nonce()
#         session["auth_nonce"] = auth_nonce
#         session["auth_opaque"] = auth_opaque
        return 'Digest realm="{0}",nonce="{1}",opaque="{2}"'.format(
            self.realm, auth_nonce, auth_opaque)
