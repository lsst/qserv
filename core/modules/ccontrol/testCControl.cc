...
BOOST_AUTO_TEST_CASE(not_like) {
    ...
    // to our antlr4 parser code we will not be able to use the antlr2 IR for testing. NOT LIKE was not
    // parsable by our antlr2 parser.
    // Instead, we test NOT LIKE purely with the antlr4 parser, by generating a statement with LIKE, and
    // we then manually change that IR to be NOT LIKE, and compare that with a generated NOT LIKE statement.
    // We know the LIKE statement is correct because we have a test that compares it with the
    // antlr2-generated LIKE statement, in antlr_compare.
    auto parserWithNot = parser::SelectParser::newInstance(
            "select shortName from Filter where shortName NOT LIKE 'Z'", parser::SelectParser::ANTLR4);
    BOOST_REQUIRE_NO_THROW(parserWithNot->setup());
    auto selectStatementWithNot = parserWithNot->getSelectStmt();
    BOOST_REQUIRE(selectStatementWithNot != nullptr);

    auto parserWithoutNot = parser::SelectParser::newInstance(
            "select shortName from Filter where shortName LIKE 'Z'", parser::SelectParser::ANTLR4);
    BOOST_REQUIRE_NO_THROW(parserWithoutNot->setup());
    auto selectStatementWithoutNot = parserWithoutNot->getSelectStmt();
    BOOST_REQUIRE(selectStatementWithoutNot != nullptr);

    // flip the 'not' on the 'without' statement to make it 'with not'
    auto whereClauseRef = selectStatementWithoutNot->getWhereClause();
    ...
    // now the the LIKE and the NOT LIKE selectStatements should be exactly the same:
    BOOST_REQUIRE_EQUAL(*selectStatementWithNot, *selectStatementWithoutNot);
}
