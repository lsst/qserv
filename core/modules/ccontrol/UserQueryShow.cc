...
LOG_LOGGER _log = LOG_GET("lsst.qserv.ccontrol.UserQueryShow");
...
UserQueryShow::UserQueryShow(UserQueryConfig const& queryConfig, UserQueryShow::Entity entity)
        : _resultTableName("qserv_show_" + queryConfig.userQueryId),
...
void UserQueryShow::submit() {
    ...
    std::string createTable = "CREATE TABLE " + _resultTableName + "(`Database` BLOB)"; // The columns must match resColumns, below.
    ...
    std::vector<std::string> resColumns({"Database"}); // this must match the schema in the CREATE TABLE statement above.
    sql::SqlBulkInsert bulkInsert(_resultDbConn.get(), _resultTableName, resColumns);
    std::vector<std::string> values = { "'one'" };
...
std::string UserQueryShow::getResultQuery() const {
