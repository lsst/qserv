...
def build(
    ...
    user : `str`
        The name of the user to run the build container as.
    """
    if pull_image and do_pull_image(qserv_image, envval_dh_user.val(), envval_dh_token.val(), dry):
        return
...
def build_build_image(
    ...
    pull_image: `bool`
        Pull the lite-qserv image from dockerhub if it exists there.
    """
    if pull_image and do_pull_image(build_image, envval_dh_user.val(), envval_dh_token.val(), dry):
        return
    images.build_image(
...
def build_run_base_image(
    ...
    if pull_image and do_pull_image(run_base_image, envval_dh_user.val(), envval_dh_token.val(), dry):
        return
    images.build_image(
...
def build_mariadb_image(
    ...
    if pull_image and do_pull_image(mariadb_image, envval_dh_user.val(), envval_dh_token.val(), dry):
        return
    images.build_image(
...
def up(
    ...
    args.extend(["up", "-d"])
    env_override = {
        envval_qserv_image.env_var: qserv_image,
        envval_mariadb_image.env_var: mariadb_image,
    }
    if dashboard_port:
        env_override[envval_dashboard_port.env_var] = str(dashboard_port)
    if dry:
