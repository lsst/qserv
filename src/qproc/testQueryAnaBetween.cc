...
BOOST_AUTO_TEST_CASE(DoubleSecondaryIndexRestrictor) {
    ...
    // std::string stmt = "select * from Object where objectIdObjTest between 38 and 40 OR objectIdObjTest IN
    // (10, 30, 70);" but this doesn't work: see DM-4017
    std::string stmt =
            "select * from Object where objectIdObjTest between 38 and 40 and objectIdObjTest IN (10, 30, "
            "70);";
    qsTest.sqlConfig =
            SqlConfig(SqlConfig::MockDbTableColumns({{"LSST", {{"Object", {"objectIdObjTest"}}}}}));
    std::shared_ptr<QuerySession> qs = queryAnaHelper.buildQuerySession(qsTest, stmt);
