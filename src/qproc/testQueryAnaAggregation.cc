...
// Aggregates in HAVING must resolve to a select list entry (or alias) so they can be rewritten in merge form.
BOOST_AUTO_TEST_CASE(HavingAggregatePlacement) {
    qsTest.sqlConfig =
            SqlConfig(SqlConfig::MockDbTableColumns({{"LSST", {{"Object", {"objectId", "bMagF"}}}}}));

    checkInternalQueries(
            *this, "select objectId, count(*) as n from LSST.Object group by objectId having round(n, 1) > 1",
            "SELECT `LSST.Object`.`objectId` AS `objectId`,count(*) AS `QS1_COUNT` "
            "FROM `LSST`.`Object_100` AS `LSST.Object` GROUP BY `objectId`",
            "SELECT `objectId` AS `objectId`,SUM(`QS1_COUNT`) AS `n` "
            "FROM `LSST`.`Object` AS `LSST.Object` GROUP BY `objectId` HAVING round(`n`,1)>1");

    checkInternalQueries(*this,
                         "select objectId, round(count(*), 1) as n from LSST.Object "
                         "group by objectId having round(count(*), 1) > 1",
                         "SELECT `LSST.Object`.`objectId` AS `objectId`,count(*) AS `QS1_COUNT` "
                         "FROM `LSST`.`Object_100` AS `LSST.Object` GROUP BY `objectId`",
                         "SELECT `objectId` AS `objectId`,round(SUM(`QS1_COUNT`),1) AS `n` "
                         "FROM `LSST`.`Object` AS `LSST.Object` GROUP BY `objectId` HAVING `n`>1");

    std::string const expectedPrefix =
            "AnalysisError:HAVING aggregate expressions must match an entry or alias in the SELECT list:";
    for (auto const& stmt : {
                 "select objectId from LSST.Object group by objectId having count(*) > 1",
                 "select objectId from LSST.Object group by objectId having round(count(*), 1) > 1",
                 "select objectId, count(*) as n from LSST.Object "
                 "group by objectId having sum(bMagF) > 1",
         }) {
        BOOST_TEST_MESSAGE("checking that this is rejected: " << stmt);
        lsst::qserv::tests::QueryAnaHelper helper;
        auto qs = helper.buildQuerySession(qsTest, stmt, true);
        BOOST_CHECK_MESSAGE(qs->getError().starts_with(expectedPrefix),
                            "expected an error starting \"" << expectedPrefix << "\" but got \""
                                                            << qs->getError() << "\"");
    }
}
