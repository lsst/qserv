...
std::shared_ptr<query::ColumnRef> makeColumnWithTable(std::string const& db, std::string const& table,
    ...
    return std::make_shared<query::ColumnRef>(std::make_shared<query::TableRef>(db, table, tableAlias),
                                              column);
}

struct TestColumns {
    TestColumns(query::ColumnRef::Ptr CRa, query::ColumnRef::Ptr CRb, bool p) : a(CRa), b(CRb), pass(p) {}

    TestColumns(std::string aDb, std::string aTable, std::string aColumn, std::string bDb, std::string bTable,
                std::string bColumn, bool p)
    ...
    TestColumns(std::string aDb, std::string aTable, std::string aTableAlias, std::string aColumn,
                std::string bDb, std::string bTable, std::string bTableAlias, std::string bColumn, bool p)
            : pass(p) {
...
struct TestColumnsAlias : public TestColumns {
    ...
    bool aliasPass;  // if the test should pass when using the TableRef alias (instead of the db & table
                     // vals);
};
...
static const std::vector<TestColumns> COLUMN_REF_MATCHES = {
        ...
        TestColumns("", "baz", "", "foo", "", "", "", "baz",
                    false),  // mismatch: don't know available table name
        TestColumns("", "baz", "a", "foo", "", "", "b", "baz",
                    false),  // mismatch: don't know available table name
        TestColumns("", "baz", "", "foo", "", "", "", "foo",
                    false),  // mismatch: don't know available table name
        TestColumns("", "baz", "a", "foo", "", "", "b", "foo",
                    false),  // mismatch: don't know available table name
        TestColumns("", "foo", "", "bar", "", "bar", "", "bar", false),    // mismatch: table
        ...
        TestColumns("foo", "bar", "", "baz", "", "", "", "baz", false),  // mismatch: can't match db or table
        TestColumns("foo", "bar", "a", "baz", "", "", "b", "baz",
                    false),  // mismatch: can't match db or table
        TestColumns("foo", "bar", "", "baz", "", "bar", "", "baz", false),       // mismatch: can't match db
...
BOOST_DATA_TEST_CASE(ColumnRefSubset, COLUMN_REF_MATCHES, columns) {
    BOOST_REQUIRE_MESSAGE(
            columns.pass == columns.a->isSubsetOf(columns.b),
            columns.a << (columns.pass ? "should " : "should NOT ") << "be a subset of " << columns.b);
}
...
BOOST_AUTO_TEST_CASE(ColumnRefComplete) {
    BOOST_REQUIRE_EQUAL(query::ColumnRef("", "", "", "column").isComplete(), false);
    BOOST_REQUIRE_EQUAL(query::ColumnRef("", "", "alias", "column").isComplete(), false);
    BOOST_REQUIRE_EQUAL(query::ColumnRef("", "table", "", "column").isComplete(), false);
    BOOST_REQUIRE_EQUAL(query::ColumnRef("", "table", "alias", "column").isComplete(), false);
    BOOST_REQUIRE_EQUAL(query::ColumnRef("db", "table", "", "column").isComplete(), false);
    BOOST_REQUIRE_EQUAL(query::ColumnRef("db", "table", "alias", "column").isComplete(), true);
}
