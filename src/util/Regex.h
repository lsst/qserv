...
public:
    ...
    public:
        ...
        void updateStr(char const* buffer) {
            _s.assign(buffer, _rmatch.rm_so, _rmatch.rm_eo - _rmatch.rm_so);
        }
    ...
    public:
        ...
        Iterator(regex_t* regex, std::string const& s) : _regex(regex), _s(s), _match(s) { setFirst(); }
        Iterator& operator++() {  // pre-increment
            /* substring found between pm.rm_so and pm.rm_eo */
            /* This call to regexec() finds the next match */
            assert(_position >= 0);
            int result = regexec(_regex, _cursor, 1, _match.regMatch(), REG_NOTBOL);
            if (result != 0)
                _position = -1;
            else {
        ...
        Match const& operator*() const { return _match; }
    ...
    Regex(char const* expr) { regcomp(&_regex, expr, 0); }
    ~Regex() { regfree(&_regex); }
    Iterator newIterator(std::string const& s) { return Iterator(&_regex, s); }
