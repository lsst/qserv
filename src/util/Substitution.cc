...
#endif
    for (std::vector<Item>::const_iterator i = _index.begin(); i != _index.end(); ++i) {
...
void Substitution::_build(std::string const& delim) {
    ...
    for (unsigned pos = _template.find(delim); pos < _template.length();
        ...
        newItem.name.assign(_template, pos + delimLength, endpos - pos - delimLength);
