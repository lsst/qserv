 ...
 */
extern "C" int luaopen_czarProxy(lua_State* L) {
...
int luaSubmitQuery(lua_State* L) {
    ...
    try {
        ...
        lsst::qserv::czar::SubmitResult res =
                lsst::qserv::proxy::submitQuery(std::string(query, lenQuery), hints);
...
int luaKillQuery(lua_State* L) {
    ...
    try {
        ...
        lsst::qserv::proxy::killQuery(std::string(query, lenQuery), std::string(clientId, lenClientId));
...
int luaLog(lua_State* L) {
    ...
    try {
        ...
        lsst::qserv::proxy::log(std::string(logger, lenLogger), std::string(level, lenLevel), filename,
                                funcname, lineno, std::string(message, lenMsg));
        return 0;
    } catch (std::exception const& exc) {
        lua_pushstring(L, exc.what());
        lua_error(L);  // lua_error() does not return
