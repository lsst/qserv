...
#include "partition/Vector.h"

using lsst::partition::Matrix3d;
using lsst::partition::Vector3d;

namespace {
bool operator==(Vector3d const& u, Vector3d const& v) { return u(0) == v(0) && u(1) == v(1) && u(2) == v(2); }

bool operator==(Matrix3d const& m, Matrix3d const& n) {
    return m.col(0) == n.col(0) && m.col(1) == n.col(1) && m.col(2) == n.col(2);
}
}  // namespace
