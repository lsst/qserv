...
InputLines const makeInputLines(ConfigStore const& config) {
    ...
    bool const in_quote = config.has("in.parq.quote") ? config.get<bool>("in.parq.quote") : false;
    bool const out_no_quote = config.has("out.csv.no-quote") ? config.get<bool>("out.csv.no-quote") : false;
    if (in_quote && !out_no_quote) {
        throw std::runtime_error(
                "Option --in.parq.quote=true can't be set along with --out.csv.no-quote=false");
    }
