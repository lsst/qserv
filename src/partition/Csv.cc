...
Dialect::Dialect(char delimiter, char escape, char quote)
        : _null(), _scanLut(new uint8_t[NUM_CHARS]), _delimiter(delimiter), _escape(escape), _quote(quote) {
    if (_quote != '\0') {
        _null = "NULL";
    } else if (_escape != '\0') {
        _null += _escape;
...
Dialect::Dialect(Dialect const &dialect)
    ...
    std::memcpy(_scanLut.get(), dialect._scanLut.get(), NUM_CHARS * sizeof(uint8_t));
}
...
size_t Dialect::decode(char *buf, char const *value, size_t size) const {
    ...
    for (; i < size && j < MAX_FIELD_SIZE; ++j) {
        ...
        if (_escape != '\0' && c == _escape) {
            if (i < size) {
                c = static_cast<char>(_unescape[static_cast<uint8_t>(value[i++])]);
            }
            ...
            if (_quote != '\0') {
                ...
                if (size > MAX_FIELD_SIZE - 2) {
                    throw std::runtime_error(
                            "CSV field value is too long to "
                            "encode.");
                }
                buf[0] = _quote;
    ...
    if (_escape != '\0') {
        for (; i < size && j < MAX_FIELD_SIZE; ++i, ++j) {
            ...
            if (c == '\r') {
                ...
                c = 'n';
            } else if (c == _delimiter || c == _escape || (c == _quote && c != '\0')) {
                buf[j++] = _escape;
                if (j == MAX_FIELD_SIZE) {
                    break;
                }
        ...
        if ((flags & HAS_CRLF) != 0) {
            throw std::runtime_error(
                    "Cannot encode CSV field with embedded "
        ...
        throw std::runtime_error(
                "Cannot encode CSV field with embedded CR, "
...
void Dialect::_validate() {
    ...
    if (_delimiter == '\0' || _delimiter == '\n' || _delimiter == '\r') {
        throw std::runtime_error(
                "The CSV field delimiter may not be set to "
                "NUL, CR or LF.");
    }
    if (_escape == _delimiter || _escape == '\n' || _escape == '\r') {
        throw std::runtime_error(
                "The CSV escape character may not be set to "
                "CR, LF or the delimiter character.");
    }
    if (_quote == _delimiter || _quote == '\n' || _quote == '\r') {
        throw std::runtime_error(
                "The CSV field quoting character may not be "
                "set to CR, LF or the delimiter character.");
    }
    if (_escape != '\0') {
        if (_escape == _quote) {
            throw std::runtime_error(
                    "The CSV escape and quote characters are "
                    "identical.");
        }
        if (_prohibited.find(_escape) != std::string::npos || _prohibited.find(_quote) != std::string::npos ||
            _prohibited.find(_delimiter) != std::string::npos) {
            throw std::runtime_error(
                    "Escaping the CSV delimiter, quote, and/"
    ...
    if ((flags & (HAS_CRLF | HAS_DELIM)) != 0) {
        throw std::runtime_error(
                "The CSV NULL representation must not contain "
...
Editor::Field::Field() : inputValue(0), outputValue(0), inputSize(0), outputSize(0), flags(0) {}
...
char const *Editor::readRecord(char const *const begin, char const *const end) {
    if (end <= begin || begin == 0) {
        ...
        throw std::runtime_error(
                "Calling readRecord() is illegal unless at "
    ...
    for (; cur < end; ++cur) {
        ...
        if (escaped) {
            ...
            if (c == _inputDialect.getEscape()) {
                ...
                if (f == fend) {
                    throw std::runtime_error(
                            "CSV record contains more than "
                            "the expected number of fields.");
                }
                f->inputValue = cur + 1;
                if (cur + 1 < end && cur[1] != '\0' && cur[1] == _inputDialect.getQuote()) {
    ...
    if (quoted || escaped) {
        throw std::runtime_error(
                "CSV record contains an embedded line "
    ...
    if (f + 1 != fend) {
        throw std::runtime_error(
                "CSV record contains less than the expected "
                "number of fields.");
    }
    ptrdiff_t sz = cur - f->inputValue;
    if (sz > MAX_FIELD_SIZE) {
...
char *Editor::writeRecord(char *buf) const {
    ...
    for (int i = 0; i < _numOutputFields; ++i) {
        ...
        if (!f.inputValue || (f.flags & Field::EDITED) != 0) {
            ...
            if (_inputDialect.isNull(f.inputValue, f.inputSize)) {
                ...
                if ((f.flags & Field::DECODE) != 0) {
                    // Decode input.
                    sz = _inputDialect.decode(decodeBuf, f.inputValue, f.inputSize);
                    val = decodeBuf;
                } else {
                    val = f.inputValue;
                    sz = f.inputSize;
        ...
        if (size + sz > MAX_LINE_SIZE - 1) {
            throw std::runtime_error(
                    "Output CSV record is longer than the "
...
bool Editor::set(int i, std::string const &val) {
    ...
    f->outputSize = static_cast<uint16_t>(_outputDialect.encode(f->outputValue, val.data(), val.size()));
    f->flags |= Field::EDITED;
    return true;
}
...
bool Editor::set(int i, char c) {
    ...
    f->outputSize = static_cast<uint16_t>(_outputDialect.encode(f->outputValue, &c, 1));
    f->flags |= Field::EDITED;
    return true;
}
...
#define IMPLEMENT_SET_IMPL(U, V, format)                                                                    \
    bool Editor::set(int i, U val) {                                                                        \
        ...
        f->outputSize =                                                                                     \
                static_cast<uint16_t>(_outputDialect.encode(f->outputValue, buf, static_cast<size_t>(sz))); \
        f->flags |= Field::EDITED;                                                                          \
        return true;                                                                                        \
    }
...
void Editor::defineOptions(po::options_description &opts) {
    ...
    out.add_options()("out.csv.field", po::value<std::vector<std::string>>(),
...
template <>
bool Editor::_get<bool>(int i) const {
    char buf[MAX_FIELD_SIZE];
    if (i < 0 || i >= _numInputFields) {
        throw std::runtime_error("Invalid input field");
    }
...
template <>
char Editor::_get<char>(int i) const {
    char buf[MAX_FIELD_SIZE];
    if (i < 0 || i >= _numInputFields) {
        throw std::runtime_error("Invalid input field");
    }
...
#define SPECIALIZE_GET_INT(U, V, suffix)                                         \
    ...
    U Editor::_get<U>(int i) const {                                             \
        ...
        } else if ((errno == ERANGE) || v > std::numeric_limits<U>::max() ||     \
