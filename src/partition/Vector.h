...
public:
    ...
    double dot(Vector3d const& v) const { return _c[0] * v._c[0] + _c[1] * v._c[1] + _c[2] * v._c[2]; }
    ...
    Vector3d const cross(Vector3d const& v) const {
        return Vector3d(_c[1] * v._c[2] - _c[2] * v._c[1], _c[2] * v._c[0] - _c[0] * v._c[2],
                        _c[0] * v._c[1] - _c[1] * v._c[0]);
    }

    /// Return the component-wise product of this vector with scalar `s`.
    Vector3d const operator*(double s) const { return Vector3d(_c[0] * s, _c[1] * s, _c[2] * s); }
...
inline Vector3d const operator*(double s, Vector3d const& v) { return v * s; }
...
public:
    ...
    Matrix3d const inverse() const {
        ...
        Vector3d a0(m(1, 1) * m(2, 2) - m(2, 1) * m(1, 2), m(1, 2) * m(2, 0) - m(2, 2) * m(1, 0),
