...
from schemaMigMgr import Migration, SchemaMigMgr, Uninitialized
...
class SchemaMigMgrTestCase(unittest.TestCase):
    ...
    def test_find_scripts(self):
        """Verify find_scripts uses the regex function to match expected
        ...
        self.assertEqual(TestMigMgr.script_match("migrate-0-to-1.sql"), (0, 1))
        self.assertEqual(TestMigMgr.script_match("migrate-foo-to-1.sql"), None)
        self.assertEqual(TestMigMgr.script_match("migrate-None-to-4.sql"), (Uninitialized, 4))
        self.assertEqual(TestMigMgr.script_match("migrate-None-to-4-somedescriptor.sql"), (Uninitialized, 4))
    ...
    def test_scripts(self):
        ...
        with TemporaryDirectory() as tmpDir:
            makeFiles(tmpDir, noneToTwo, oneToTwo, "unused.sql")
            migMgr = TestMigMgr(tmpDir)
            self.assertEqual(set(migMgr.migrations),
                             set((Migration(Uninitialized, 2, noneToTwo, os.path.join(tmpDir, noneToTwo)),
                                  Migration(1, 2, oneToTwo, os.path.join(tmpDir, oneToTwo)))))

    def test_migration_latest_version(self):
        """Verify the Migration less than operator works as expected."""
    ...
    def test_migration_path(self):
        ...
        self.assertEqual(SchemaMigMgr.migration_path(Uninitialized, 4, [uninitToFour]),
                         [uninitToFour])
        # test uninit to 2, 2 to 4.
        self.assertEqual(SchemaMigMgr.migration_path(Uninitialized, 4, [uninitToTwo, twoToFour]),
                         [uninitToTwo, twoToFour])
        # test that uninit to 4 is chosen over uninit to 2 with 2 to 4
        self.assertEqual(SchemaMigMgr.migration_path(Uninitialized, 4, [uninitToTwo, twoToFour, uninitToFour]),
                         [uninitToFour])
        # test that uninit to 2, 2 to 4 is chosen over uninit to 1, 1 to 2, 2 to 4.
        self.assertEqual(SchemaMigMgr.migration_path(Uninitialized, 4, [uninitToOne, uninitToTwo, twoToFour]),
                         [uninitToTwo, twoToFour])
        # test 2 to 4 out of the crowded field.
        self.assertEqual(SchemaMigMgr.migration_path(2, 4, [uninitToOne, uninitToTwo, twoToFour, uninitToFour]),
                         [twoToFour])
        # test going to an unsupported version.
        self.assertEqual(SchemaMigMgr.migration_path(2, 5, [uninitToOne, uninitToTwo, twoToFour, uninitToFour]),
                         [])
    ...
    def test_migrate(self):
        ...
        with TemporaryDirectory() as scriptsDir:
            noneToThree = Migration(Uninitialized, 3, "migrate-None-to-3.sql",
                                    os.path.join(scriptsDir, "migrate-None-to-3.sql"))
            noneToOne = Migration(Uninitialized, 1, "migrate-None-to-1.sql",
                                  os.path.join(scriptsDir, "migrate-None-to-1.sql"))
            oneToTwo = Migration(1, 2, "migrate-1-to-2.sql",
                                  os.path.join(scriptsDir, "migrate-1-to-2.sql"))
            twoToThree = Migration(2, 3, "migrate-2-to-3.sql",
            ...
            # add a short 1-to-3
            oneToThree = Migration(1, 3, "migrate-1-to-3.sql",
