...
SchedulerBase::SchedulerBase(std::string const& name, int maxThreads, int maxReserve, int maxActiveChunks, int priority)
    : _name{name},
    ...
    LOGS(_log, LOG_LVL_INFO, "Scheduler name=" << name << " maxThreads=" << _maxThreads << " maxThreads=" << _maxThreads << " priority=" << _priority);
}
