...
namespace {
LOG_LOGGER _log = LOG_GET("lsst.qserv.protojson.JobReadyMsg");
}  // namespace

namespace lsst::qserv::protojson {

JobReadyMsg::Ptr JobReadyMsg::create(std::string const& replicationInstanceId,
                                     ...
                                     UberJobId uberJobId, std::string const& fileUrl, uint64_t rowCount,
                                     uint64_t fileSize) {
    auto jrMsg = Ptr(new JobReadyMsg(replicationInstanceId, replicationAuthKey));
    jrMsg->_workerId = workerIdStr;
    jrMsg->_czarName = czarName;
    jrMsg->_czarId = czarId;
    jrMsg->_queryId = queryId;
    jrMsg->_uberJobId = uberJobId;
    jrMsg->_fileUrl = fileUrl;
    jrMsg->_rowCount = rowCount;
    jrMsg->_fileSize = fileSize;
    return jrMsg;
}

JobReadyMsg::Ptr JobReadyMsg::createFromJson(nlohmann::json const& jsWReq,
                                             std::string const& replicationInstanceId,
                                             std::string const& replicationAuthKey) {
    string const fName("JobReadyMsg::createFromJson");
    ...
    try {
        ...
        jrMsg->_fileUrl = http::RequestBodyJSON::required<string>(jsWReq, "fileUrl");
        jrMsg->_rowCount = http::RequestBodyJSON::required<uint64_t>(jsWReq, "rowCount");
        jrMsg->_fileSize = http::RequestBodyJSON::required<uint64_t>(jsWReq, "fileSize");
        return jrMsg;
    } catch (invalid_argument const& exc) {
        LOGS(_log, LOG_LVL_ERROR, string("JobReadyMsg::createJson invalid ") << exc.what());
    }
    return nullptr;
}

json JobReadyMsg::serializeJson() {
