...
void MergingHandler::errorFlush(std::string const& msg, int code) {
    _setError(code, util::Error::NONE, msg, util::Error::RESULT_IMPORT);
...
qdisp::MergeEndStatus MergingHandler::_mergeHttp(qdisp::UberJob::Ptr const& uberJob, string const& fileUrl,
    ...
    if (!fileMergeSuccess) {
        ...
        _setError(ccontrol::MSG_RESULT_ERROR, util::Error::NONE, err.getMsg(), util::Error::RESULT_IMPORT);
    }
    if (csvMemDisk->getContaminated()) {
        LOGS(_log, LOG_LVL_ERROR, __func__ << " merge stream contaminated");
        fileMergeSuccess = false;
        _setError(ccontrol::MSG_RESULT_ERROR, util::Error::NONE, "merge stream contaminated",
                  util::Error::RESULT_IMPORT);
    }

    if (!fileReadErrorMsg.empty()) {
        LOGS(_log, LOG_LVL_WARN, __func__ << " result file read failed");
        _setError(ccontrol::MSG_HTTP_RESULT, util::Error::NONE, fileReadErrorMsg, util::Error::RESULT_IMPORT);
    }
    _flushed = true;
...
void MergingHandler::_setError(int code, int subError, std::string const& msg, int errorState) {
    LOGS(_log, LOG_LVL_DEBUG, "_setError: code: " << code << ", message: " << msg);
    auto exec = _executive.lock();
    if (exec == nullptr) return;
    exec->addMultiError(code, subError, msg, errorState);
}
