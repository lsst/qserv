...
void readHttpFileAndMerge(string const& httpUrl, size_t fileSize,
    ...
    try {
        ...
        if (offset != fileSize) {
            throw runtime_error(context + "short read of the message header at offset: " + to_string(offset) +
                                ", fileSize: " + to_string(fileSize) + ", file: " + httpUrl);
        }
...
bool jobNoLongerActive(shared_ptr<qdisp::JobQuery> const& jobQuery) {
...
bool MergingHandler::_merge(proto::ResponseSummary const& responseSummary,
    ...
    if (jobNoLongerActive(jobQuery)) return true;
    ...
    thread csvThread([jobQuery, csvStream, responseSummary]() {
        size_t bytesRead = 0;
    ...
    bool const success = _infileMerger->merge(responseSummary, csvStream);
    csvThread.join();
    _flushed = true;
    if (!success) {
