 ...
 *
 */
struct HyriseTestQueries {
    /**
     * @brief Construct a new Hyrise Test Queries object
     *
     ...
     *                         the IR.
     */
    HyriseTestQueries(string const& iQuery, function<shared_ptr<query::SelectStmt>()> const& iCompareStmt,
                      string const& iSerializedQuery)
            : query(iQuery), compareStmt(iCompareStmt), serializedQuery(iSerializedQuery) {}

    /// query to test, that will be turned into a SelectStmt by the parser.
    string query;

    /// comparison query, that will be turned into a SelectStmt by the parser and then that will
...
ostream& operator<<(ostream& os, HyriseTestQueries const& i) {
    os << "HyriseTestQueries(" << i.query << "...)";
    return os;
}

static const vector<HyriseTestQueries> HYRISE_TEST_QUERIES = {
                            ...
                            WhereClause(OrTerm(AndTerm(
                                    BoolFactor(
                                            ...
                                            CompPredicate(ValueExpr("", FactorOp(ValueFactor(ColumnRef(
                                                          ...
                                                          ValueExpr("", FactorOp(ValueFactor("887404831"),
                                                                                 query::ValueExpr::NONE)))),
                                    BoolFactor(IS,
                                               CompPredicate(
                                                       ValueExpr("", FactorOp(ValueFactor(ColumnRef(
                                                                              ...
                                                                              query::ValueExpr::NONE)),
                                                       query::CompPredicate::EQUALS_OP,
                                                       ValueExpr("", FactorOp(ValueFactor("'3,3'"),
                                                                              query::ValueExpr::NONE)))),
                                    BoolFactor(IS,
                                               LikePredicate(
                                                       ValueExpr("", FactorOp(ValueFactor(ColumnRef(
                                                                              ...
                                                                              query::ValueExpr::NONE)),
                                                       LIKE,
                                                       ValueExpr("", FactorOp(ValueFactor("'%'"),
                                                                              query::ValueExpr::NONE))))))),
        ...
        HyriseTestQueries(
                ...
                "GROUP BY `objectId` ORDER BY `ra` ASC"),
        // test multiple JOIN
        // test ASC
        HyriseTestQueries(
                ...
                "qserv_areaspec_box(355,0,360,20)`filterName`='g' ORDER BY `objectId` ASC, `taiMidPoint` "
                "ASC"),
        // test hex
        HyriseTestQueries(
        ...
        HyriseTestQueries(
                ...
                "SELECT `objectId` FROM `Object` WHERE qserv_areaspec_box(0,0,3,10) ORDER BY `objectId` ASC"),
        // test qserv_areaspec_box wrapped in an equality comparison (often written that way in upstream ADQL)
        HyriseTestQueries(
        ...
        HyriseTestQueries(
                ...
                "IN(386942193651348) ORDER BY `Source`.`sourceId` ASC"),
        ...
        HyriseTestQueries(
                ...
                "`objectId`>430000000000000 ORDER BY `objectId` ASC"),
                    ...
                    return SelectStmt(
                            ...
                            GroupByClause(GroupByTerm(
                                    ValueExpr("", FactorOp(ValueFactor(ColumnRef("", "", "objectId")),
                                                           query::ValueExpr::NONE)),
                                    "")),
                            HavingClause(AndTerm(BoolFactor(
                                    IS,
                                    CompPredicate(
                                            ValueExpr(
                                                    "",
        ...
        // test a function nested inside an ORDER BY expression. Bare "ORDER BY f(x)" is rejected, but a
        // function within a larger expression is allowed by the parser (could still be rejected by qana).
        HyriseTestQueries(
                ...
                "SELECT `objectId` FROM `Object` ORDER BY(`ra_PS`+scisql_fluxToAbMag(`gFlux_PS`)) ASC"),
...
BOOST_DATA_TEST_CASE(hyrise_test, HYRISE_TEST_QUERIES, queryInfo) {
    ...
    // Verify the Hyrise parser renders the SQL form we expect.
...
// A bare function call as the entire ORDER BY expression is rejected. This is a deliberate design
// decision: see doc/user/sql.rst's "ORDER BY expressions must appear in the SELECT list" section,
// which suggests using aliases as a workaround.
static std::vector<std::string> const ORDER_BY_FUNCTION_QUERIES = {
...
static std::vector<std::string> const HAVING_QUERIES = {
        "SELECT objectId, COUNT(*) FROM Object GROUP BY objectId HAVING COUNT(*) > 1",
        "SELECT COUNT(*) FROM Object HAVING COUNT(*) > 1",
        "SELECT objectId, MAX(raFlux) - MIN(raFlux) FROM Source GROUP BY objectId "
        "HAVING MAX(raFlux) - MIN(raFlux) > 5",
        "SELECT objectId, MAX(raFlux) - MIN(raFlux) AS flx FROM Source GROUP BY objectId "
        "HAVING MAX(raFlux) - MIN(raFlux) > 5",
        "SELECT objectId, MAX(raFlux) - MIN(raFlux) AS flx FROM Source GROUP BY objectId HAVING flx > 5",
        "SELECT objectId, MAX(raFlux) - MIN(raFlux) AS flx FROM Source GROUP BY objectId "
        "HAVING flx > 5 LIMIT 5",
...
static std::vector<std::string> const AGGREGATE_ARGUMENT_SUPPORTED_QUERIES = {
        "SELECT COUNT(objectId) FROM Object",
...
static std::vector<std::string> const SCHEMA_QUALIFIED_FUNCTION_QUERIES = {
        "SELECT custom.myfunc(ra_PS) FROM Object",
        "SELECT custom.MAX(ra_PS) FROM Object",
...
void checkAreaRestrictorConjunctionIr(std::string const& query, std::string const& expectedRootTerm) {
    ...
    auto& whereClause = selectStmt->getWhereClause();
    BOOST_REQUIRE(whereClause.hasRestrs());
    BOOST_REQUIRE(whereClause.getRestrs() != nullptr);
    BOOST_REQUIRE_EQUAL(whereClause.getRestrs()->size(), 1);
    BOOST_REQUIRE(whereClause.getRootTerm() != nullptr);

    query::QueryTemplate rootTermTemplate;
    whereClause.getRootTerm()->renderTo(rootTermTemplate);
...
BOOST_AUTO_TEST_CASE(area_restrictor_in_conjunction_supported) {
    checkAreaRestrictorConjunctionIr(
            ...
            "`filterName`='g' AND `ra_PS`>1");
    checkAreaRestrictorConjunctionIr(
            "SELECT objectId FROM Object "
            "WHERE qserv_areaspec_box(0, 0, 3, 10) AND (filterName='g' OR filterName='r')",
            "`filterName`='g' OR `filterName`='r'");
    checkAreaRestrictorConjunctionIr(
            "SELECT objectId FROM Object "
            "WHERE qserv_areaspec_box(0, 0, 3, 10) AND NOT (filterName='g')",
            "NOT `filterName`='g'");
}
