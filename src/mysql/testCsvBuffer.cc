...
BOOST_AUTO_TEST_CASE(TestCsvBuffer) {
    std::string const inBuffer = "abc\tdef\tghi\njkl\tmno\tpqr\n";
    std::shared_ptr<lsst::qserv::mysql::CsvBuffer> csvBuf =
            lsst::qserv::mysql::newForwardedCsvBuffer(inBuffer.data(), inBuffer.size());
    BOOST_CHECK_EQUAL(csvBuf->fetch(nullptr, 0), 0);

    std::string outBuffer;

    outBuffer.resize(8);
    BOOST_CHECK_EQUAL(csvBuf->fetch(outBuffer.data(), outBuffer.size()), outBuffer.size());
    BOOST_CHECK_EQUAL(outBuffer, "abc\tdef\t");

    outBuffer.resize(8);
    BOOST_CHECK_EQUAL(csvBuf->fetch(outBuffer.data(), outBuffer.size()), outBuffer.size());
    BOOST_CHECK_EQUAL(outBuffer, "ghi\njkl\t");

    outBuffer.resize(10);
    BOOST_CHECK_EQUAL(csvBuf->fetch(outBuffer.data(), outBuffer.size()), 8);

    outBuffer.resize(8);
    BOOST_CHECK_EQUAL(outBuffer, "mno\tpqr\n");

    BOOST_CHECK_EQUAL(csvBuf->fetch(outBuffer.data(), outBuffer.size()), 0);
}
