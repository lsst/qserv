...
/// CsvBuffer objects are used to buffer rows for LocalInfile, and because
...
class ForwardedCsvBuffer : public CsvBuffer {
public:
    ForwardedCsvBuffer(char const* buf, uint32_t size) : _buf(buf), size(size) {}

    unsigned fetch(char* buffer, unsigned bufLen) override {
        if (_offset >= size) {
            return 0;
        }
        unsigned const bytesToCopy = std::min(bufLen, size - _offset);
        ::memcpy(buffer, _buf + _offset, bytesToCopy);
        _offset += bytesToCopy;
        return bytesToCopy;
    }

    std::string dump() const override { return "ForwardedCsvBuffer size=" + std::to_string(size); }
...
std::shared_ptr<CsvBuffer> newForwardedCsvBuffer(char const* buf, uint32_t size) {
    return std::make_shared<ForwardedCsvBuffer>(buf, size);
}
