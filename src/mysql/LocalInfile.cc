...
#if 1 //&&&
std::map<std::string, int> InstanceMCount::_instances;
std::recursive_mutex InstanceMCount::_mx;

InstanceMCount::InstanceMCount(std::string const& className) : _className{className} { _increment("con"); }

InstanceMCount::InstanceMCount(InstanceMCount const& other) : _className{other._className} { _increment("cpy"); }

InstanceMCount::InstanceMCount(InstanceMCount&& origin) : _className{origin._className} { _increment("mov"); }

std::atomic<uint64_t> instLogLimiter{0}; //&&&

void InstanceMCount::_increment(std::string const& source) {
    ...
    if ((++instLogLimiter)%10000 == 0) {
        LOGS(_log, LOG_LVL_WARN, "&&& InstanceMCount brief ");
        LOGS(_log, LOG_LVL_WARN, "&&& InstanceMCount brief " << iter->first << "=" << iter->second << " : " << *this);
    }

}


InstanceMCount::~InstanceMCount() {
    LOGS(_log, LOG_LVL_WARN, "~InstanceMCount &&&tst");
    std::lock_guard<std::recursive_mutex> lg(_mx);
    auto iter = _instances.find(_className);
    if (iter != _instances.end()) {
        iter->second -= 1;
        LOGS(_log, LOG_LVL_WARN, "~InstanceMCount " << iter->first << "=" << iter->second << " : " << *this);
        if (iter->second == 0) {
            _instances.erase(_className);
            //LOGS(_log, LOG_LVL_WARN, "~InstanceMCount &&& " << iter->first << "=" << iter->second << " : " << *this);
        }
    } else {
        LOGS(_log, LOG_LVL_ERROR, "~InstanceMCount " << _className << " was not found! : " << *this);
    }

}

int InstanceMCount::getCount() {
    std::lock_guard<std::recursive_mutex> lg(_mx);
    auto iter = _instances.find(_className);
    if (iter == _instances.end()) {
        return 0;
...
std::ostream& operator<<(std::ostream& os, InstanceMCount const& instanceCount) {
