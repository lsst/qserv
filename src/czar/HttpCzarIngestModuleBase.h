...
protected:
     ...
     *   The second input parameter of the function is the mapping of chunk IDs to worker IDs (the map
     *   will match a collection of the chunk IDs in the parameter chunkIds).
    ...
    std::list<std::pair<std::string, std::string>> ingestData(
            ...
            std::function<std::map<std::string, std::string>(
                    std::uint32_t, std::map<std::int32_t, std::string> const&)> const&
...
private:
     ...
     * @return A map of chunks and the corresponding worker identifiers.
     * @throw http::Error In case of a communication error or an error reported by the server.
     */
    std::map<std::int32_t, std::string> _allocateChunks(std::set<std::int32_t> const& chunkIds,
