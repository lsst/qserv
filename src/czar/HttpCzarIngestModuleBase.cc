...
list<pair<string, string>> HttpCzarIngestModuleBase::ingestData(
        ...
        function<map<string, string>(uint32_t, map<int32_t, string> const&)> const& submitRequestsToWorkers) {
    ...
    map<int32_t, string> chunk2workerId;
    if (!chunkIds.empty()) {
        chunk2workerId = _allocateChunks(chunkIds, databaseName);
    }

    map<string, string> const workerErrors = submitRequestsToWorkers(transactionId, chunk2workerId);
...
map<int32_t, string> HttpCzarIngestModuleBase::_allocateChunks(set<int32_t> const& chunkIds,
    ...
    json const response = _requestController(http::Method::POST, "/ingest/chunks", data);
    map<int32_t, string> result;
    for (auto const& entry : response.at("location")) {
        result[entry.at("chunk").get<int32_t>()] = entry.at("worker").get<string>();
    }
    return result;
}
...
void HttpCzarIngestModuleBase::_unpublishOrCreateDatabase(const string& databaseName) {
    ...
    for (const auto& database : config.at("databases")) {
        if (boost::iequals(database.at("database").get<string>(), databaseName)) {
            ...
            if (::countDirectors(database) == 0) _createDefaultDirectorTable(databaseName);
            return;
        }
    }
