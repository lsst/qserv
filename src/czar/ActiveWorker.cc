...
void ActiveWorker::setWorkerContactInfo(protojson::WorkerContactInfo::Ptr const& wcInfo) {
    LOGQ(_log, LOG_LVL_INFO, cName(__func__) << " new info=" << wcInfo->dump());
    lock_guard<mutex> lg(_aMtx);
...
void ActiveWorker::updateStateAndSendMessages(double timeoutAliveSecs, double timeoutDeadSecs,
                                              double maxLifetime) {
    LOGQ(_log, LOG_LVL_TRACE, cName(__func__) << " start");
    ...
    protojson::WorkerContactInfo::Ptr wInfo_;
    {
        ...
        if (wInfo_ == nullptr) {
            LOGQ(_log, LOG_LVL_ERROR, cName(__func__) << " no WorkerContactInfo");
            return;
        }
    ...
    LOGQ(_log, LOG_LVL_DEBUG, cName(__func__) << " queuing message");
...
void ActiveWorker::_sendStatusMsg(protojson::WorkerContactInfo::Ptr const& wInf,
    ...
    if (wInf == nullptr) {
        LOGQ(_log, LOG_LVL_ERROR, cName(__func__) << " wInfo was null.");
        return;
    }
    ...
    LOGQ(_log, LOG_LVL_DEBUG, cName(__func__) << " REQ " << jsWorkerReq);
    ...
    try {
        ...
        if (respMsg->success) {
            ...
            if (startupTimeChanged) {
                LOGQ(_log, LOG_LVL_WARN, cName(__func__) << " worker startupTime changed, likely rebooted.");
                // kill all incomplete UberJobs on this worker.
                czar::Czar::getCzar()->killIncompleteUbjerJobsOn(wInf->wId);
            }
        } else {
            LOGQ(_log, LOG_LVL_ERROR, cName(__func__) << " transmit failure:" << *respMsg);
        }
    } catch (exception const& ex) {
        LOGQ(_log, LOG_LVL_ERROR, requestContext + " transmit failure, ex: " + ex.what());
