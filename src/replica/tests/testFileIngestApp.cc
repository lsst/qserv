...
BOOST_AUTO_TEST_CASE(FileIngestAppTest_parseChunkContribution) {
    ...
    BOOST_REQUIRE_NO_THROW({ contrib = FileIngestApp::parseChunkContribution("chunk_1.txt"); });
    BOOST_CHECK(contrib.chunk == 1);
    BOOST_CHECK(not contrib.isOverlap);

    BOOST_REQUIRE_NO_THROW({ contrib = FileIngestApp::parseChunkContribution("chunk_2_overlap.txt"); });
    ...
    BOOST_CHECK_THROW({ contrib = FileIngestApp::parseChunkContribution("test_2.txt"); }, invalid_argument);
