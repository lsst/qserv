...
BOOST_AUTO_TEST_CASE(ReplicaInfoTest) {
    ...
    BOOST_CHECK(inFirstOnly.size() == 2 && (inFirstOnly[0].chunk == 1) &&
                (inFirstOnly[0].database == "db2") && (inFirstOnly[1].chunk == 3) &&
                (inFirstOnly[1].database == "db3"));

    QservReplicaCollection inSecondOnly;
    BOOST_CHECK(diff(two, one, inSecondOnly));
    BOOST_CHECK(inSecondOnly.size() == 2);
    BOOST_CHECK(inSecondOnly.size() == 2 && (inSecondOnly[0].chunk == 3) &&
                (inSecondOnly[0].database == "db2") && (inSecondOnly[1].chunk == 4) &&
                (inSecondOnly[1].database == "db3"));
...
BOOST_AUTO_TEST_CASE(ReplicaInfoFileInfoTest) {
    ...
    auto const makeFileInfo = [&](string const& name) -> ReplicaInfo::FileInfo {
        return ReplicaInfo::FileInfo{name, size, mtime, cs, beginTransferTime, endTransferTime, inSize};
    };
