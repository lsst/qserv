...
json ConfigSchemaWorker::_build() {
    json schema = json::object();
    for (auto const& category : {"common", "database", "registry", "security"}) {
        schema[category] = ConfigSchema::sharedSchema(category);
    }

    // Add Worker-specific parameters to the category.
    json& database = schema["database"];
    database["worker-db-conn"] = {
            {"description", "A connection URL for the MySQL service of the Qserv worker database."},
            {"security-context", 1},
            {"default", "mysql://qsmaster@localhost:3306/qservw_worker"}};

    // This is the Worker-specific category of the schema.
    schema["worker"] = json::object();
    json& worker = schema["worker"];
    ...
    worker["async-loader-auto-resume"] = {
            ...
            {"empty-allowed", 1},
            {"default", 1}};
    worker["async-loader-cleanup-on-resume"] = {
            {"description",
             "The flag controlling the behavior of Replication worker's ASYNC ingest service after"
    ...
    worker["http-max-queued-requests"] = {
            {"description",
            ...
            {"empty-allowed", 1},
            {"default", 0}};
    worker["exporter-max-queued-requests"] = {
            {"description",
             "The maximum number of pending requests, i.e. requests accept()ed by"
            ...
            {"empty-allowed", 1},
            {"default", 0}};
    worker["http-svc-max-queued-requests"] = {
            {"description",
             "The maximum number of pending requests, i.e. requests accept()ed by"
            ...
            {"empty-allowed", 1},
            {"default", 0}};
    worker["svc-port"] = {{"description", "The port number for the worker replication service."},
                          {"default", 25000}};
    worker["http-svc-port"] = {
            {"description", "The port number for the HTTP-based worker replication service."},
            {"default", 25005}};
    worker["fs-port"] = {{"description", "The port number for the worker's file service."},
                         {"default", 25001}};
    worker["data-dir"] = {{"description",
    ...
    worker["exporter-port"] = {
            ...
            {"default", 25003}};
    worker["exporter-tmp-dir"] = {{"description",
    ...
    worker["http-loader-port"] = {
            ...
            {"default", 25004}};
    worker["http-loader-tmp-dir"] = {{"description",
    ...
    worker["ingest-max-retries"] = {
            ...
            {"empty-allowed", 1},
            {"default", 10}};
    worker["director-index-record-size"] = {
            {"description",
    ...
    worker["create-databases-on-scan"] = {
            {"description",
            ...
            {"default", 1},
            {"empty-allowed", 1}};
    worker["create-folders"] = {
            {"description",
             "The flag (if it's not 0) tells the Worker to create missing folders used"
             " by the Worker. It's suggested to disable this feature in the production deployments"
...
ConfigSchemaWorker::ConfigSchemaWorker() : ConfigSchema(ConfigSchemaWorker::_build()) {}
