...
private:
    ...
    /// The relative name of a temporary folder where the file will be stored.
    /// The folder gets created before extracting data from the MySQL table into the file.
    /// It's expected that the folder will be deleted by the worker upon a request from
    /// the Controller after it finishes constructing the director index of the table.
    std::string const _tmpDirName;

    /// The full path name of the temporary folder where the file will be stored.
    std::string const _tmpDirFullPath;

    // The name of the temporary file within the folder into which the TSV/CSV dump will be made.
    std::string const _fileName;

    /// The full path name of a temporary file into which the TSV/CSV dump will be made.
    /// It's expected that the file will be deleted by the worker upon a request from the Controller
    /// after it finishes pulling the file from the work and ingesting the file into the director
    /// index of the table.
    std::string const _fullFilePath;

    /// The size of the file is determined after finishing extracting data from MySQL.
