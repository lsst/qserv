...
bool WorkerHttpProcessorThread::isRunning() const { return _isRunning(replica::Lock(_mtx)); }

void WorkerHttpProcessorThread::run() {
    replica::Lock lock(_mtx);
    if (_isRunning(lock)) return;
        ...
        while (!self->_stop) {
            ...
            if (request) {
                ...
                try {
                    ...
                    request->setStatus(protocol::Status::FAILED, protocol::StatusExt::CONFIG_NO_SUCH_DB);
                    finished = true;
                    ...
                    request->setStatus(protocol::Status::FAILED, protocol::StatusExt::CONFIG_NO_SUCH_TABLE);
                    finished = true;
                } catch (std::exception const& ex) {
                    LOGS(_log, LOG_LVL_ERROR,
                         self->context() << "failed processing"
                                         << "  id: " << request->id() << "  exception: " << ex.what());
                    request->setStatus(protocol::Status::FAILED, protocol::StatusExt::OTHER_EXCEPTION);
                    finished = true;
                }
                if (finished) {
                    LOGS(_log, LOG_LVL_DEBUG,
...
void WorkerHttpProcessorThread::stop() {
    replica::Lock lock(_mtx);
    if (_isRunning(lock) && !_stop) _stop = true;
}

bool WorkerHttpProcessorThread::_isRunning(replica::Lock const& lock) const { return _thread != nullptr; }
...
void WorkerHttpProcessorThread::_stopped() {
    ...
    // Notify the processor from the lock-free context to avoid potential deadlocks. The processor
    // will check the state of all threads by calling isRunning() on each thread and complete
    // the state transition if all threads are stopped.
