...
WorkerSqlHttpRequest::WorkerSqlHttpRequest(shared_ptr<ServiceProvider> const& serviceProvider,
    ...
    _binaryEncodingMode = http::parseBinaryEncoding(params.optionalString("binary_encoding", "hex"));
    ...
    switch (_sqlRequestType) {
        ...
        case protocol::SqlRequestType::CREATE_DATABASE:
            _ifNotExists = params.optionalBool("if_not_exists", _ifNotExists);
            break;
        case protocol::SqlRequestType::DROP_DATABASE:
            _ifExists = params.optionalBool("if_exists", _ifExists);
            break;
        case protocol::SqlRequestType::CREATE_TABLE:
            _tables = _getTables(params, _batchMode);
...
json WorkerSqlHttpRequest::getResult() const { return {{"result_sets", _resultSets}}; }
...
vector<Query> WorkerSqlHttpRequest::_queries(Connection::Ptr const& conn) const {
    ...
    switch (_sqlRequestType) {
        case protocol::SqlRequestType::QUERY:
            ...
            break;
        case protocol::SqlRequestType::CREATE_DATABASE: {
            string const query = g.createDb(_databaseName, _ifNotExists);
            queries.emplace_back(Query(query));
            break;
        }
        case protocol::SqlRequestType::DROP_DATABASE: {
            string const query = g.dropDb(_databaseName, _ifExists);
            queries.emplace_back(Query(query));
            break;
        }
        case protocol::SqlRequestType::ENABLE_DATABASE: {
            ...
            string const query = g.replace("qservw_worker", "Dbs", _databaseName);
            queries.emplace_back(Query(query));
            break;
        }
        case protocol::SqlRequestType::DISABLE_DATABASE: {
            string const where = g.where(g.eq("db", _databaseName));
        ...
        default:
            ...
            queries.emplace_back(_generateQuery(conn, _tables[0]));
            break;
    }
    return queries;
}
...
Query WorkerSqlHttpRequest::_generateQuery(Connection::Ptr const& conn, string const& table) const {
    QueryGenerator const g(conn);
    SqlId const databaseTable = g.id(_databaseName, table);
    switch (_sqlRequestType) {
        case protocol::SqlRequestType::CREATE_TABLE: {
            list<string> const keys;
            string query = g.createTable(databaseTable, _ifNotExists, _columns, keys, _engine, _comment,
        ...
        case protocol::SqlRequestType::DROP_TABLE: {
            string const query = g.dropTable(databaseTable, _ifExists);
            return Query(query, databaseTable.str);
        }
        case protocol::SqlRequestType::DROP_TABLE_PARTITION: {
            string const query = g.alterTable(databaseTable) + g.dropPartition(_transactionId, _ifExists);
            return Query(query, databaseTable.str);
        }
        case protocol::SqlRequestType::REMOVE_TABLE_PARTITIONING: {
        ...
        case protocol::SqlRequestType::CREATE_TABLE_INDEX: {
            string const query = g.createIndex(databaseTable, _index.name, _index.spec, _index.keys,
                                               _ifNotExists, _index.comment);
            return Query(query, databaseTable.str);
        }
        case protocol::SqlRequestType::DROP_TABLE_INDEX: {
            string const query = g.dropIndex(databaseTable, _indexName, _ifExists);
            return Query(query, databaseTable.str);
        }
        case protocol::SqlRequestType::GET_TABLE_INDEX: {
        ...
        case protocol::SqlRequestType::TABLE_ROW_STATS: {
            ...
            string query = g.select(Sql::COUNT_STAR) +
                           ...
                           g.where(g.eq("TABLE_SCHEMA", _databaseName), g.eq("TABLE_NAME", table),
...
void WorkerSqlHttpRequest::_extractResultSet(replica::Lock const& lock, Connection::Ptr const& conn) {
    ...
    if (conn->hasResult()) {
        ...
        while (conn->next(row)) {
            ...
            rowsJson.push_back(row.toJson(_binaryEncodingMode));
        }
    }
}
...
void WorkerSqlHttpRequest::_reportFailure(replica::Lock const& lock, protocol::StatusExt statusExt,
    ...
    if (_batchMode) {
        ...
        // For non-batch requests the failure of a single query is considered as a failure
        // of the whole request. Hence we're reporting the error message as well.
        setStatus(lock, protocol::Status::FAILED, statusExt, error);
    }
}
