...
public:
    ...
    /**
     * Tell the running thread to abort processing the current request (if any), put that request
     * back into the input queue, stop fetching new requests and finish. The thread can be resumed
     * later by calling method run().
     *
     * @note This operation may have a delayed effect. The thread will stop asynchronously after
     * it finishes processing the current request. If the thread is idle at the moment of calling
     * this method then it will stop immediately. The client may want to call the method isRunning()
     * to check if the thread has actually stopped.
     */
    void stop();
...
private:
    ...
    /// @return 'true' if the processing thread is still running
    /// @note this method is supposed to be called within the scope of a lock on _mtx
    bool _isRunning(replica::Lock const& lock) const;
    ...
    mutable replica::Mutex _mtx;  /// Mutex guarding operations with the thread
