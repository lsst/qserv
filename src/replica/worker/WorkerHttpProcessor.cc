...
bool WorkerHttpProcessor::PriorityQueueType::exists(string const& id) const {
...
json WorkerHttpProcessor::statusOfRequests(string const& id) {
...
bool WorkerHttpProcessor::_exists(replica::Lock const& lock, string const& id) const {
    // The ordering of the operations is important for performance of the algorithm since
    // the check in the first two collections is O(log n) while the check in the new requests
    // queue is O(n), where n is the number of requests in the queue.
    return _finishedRequests.count(id) > 0 || _inProgressRequests.count(id) > 0 || _newRequests.exists(id);
}
