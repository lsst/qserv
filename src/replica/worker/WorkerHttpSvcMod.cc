...
protocol::QueuedRequestHdr WorkerHttpSvcMod::_parseHdr(string const& func) const {
    auto const hdrJson = body().required<json>("hdr");
    protocol::QueuedRequestHdr const hdr(body().required<string>(hdrJson, "id"),
                                         body().optional<int>(hdrJson, "priority", 0),
                                         body().optional<unsigned int>(hdrJson, "timeout", 0));
