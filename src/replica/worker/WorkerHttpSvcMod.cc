...
json WorkerHttpSvcMod::executeImpl(string const& subModuleName) {
    ...
    if (subModuleName == "ECHO")
        ...
        return _sql();
    else if (subModuleName == "INDEX-GET")
        return _getIndex();
    else if (subModuleName == "INDEX-DELETE")
        return _deleteIndex();
...
protocol::QueuedRequestHdr WorkerHttpSvcMod::_parseHdr(string const& func) const {
    protocol::QueuedRequestHdr const hdr(body().requiredString("id"), body().optionalInt("priority", 0),
                                         body().optionalUInt("timeout", 0));
...
protocol::RequestParams WorkerHttpSvcMod::_reqParams() const {
    return protocol::RequestParams(body().required<json>("params"));
}
...
json WorkerHttpSvcMod::_getIndex() {
    debug(__func__);
    checkApiVersion(__func__, 56);
    return _processor->index(_parseHdr(__func__), _reqParams());
}

json WorkerHttpSvcMod::_deleteIndex() {
    debug(__func__);
    checkApiVersion(__func__, 56);
    string const folder = params().at("folder");
    string const file = params().at("file");
    debug(__func__, "folder: '" + folder + "'");
    debug(__func__, "file: '" + file + "'");

    // (Security measure) Sanitize and validate the path for correctness. The path is expected to be relative
    ...
    protocol::StatusExt statusExt = protocol::StatusExt::NONE;
    string errorMsg;
    std::error_code ec;
    if (!fs::exists(fullFilePath, ec)) {
        status = protocol::Status::FAILED;
        statusExt = protocol::StatusExt::NO_FILE;
        errorMsg = "file '" + file + "' in folder '" + folder + "' does not exist";
    } else {
        fs::remove(fullFilePath, ec);
        if (ec.value() != 0) {
            status = protocol::Status::FAILED;
            statusExt = protocol::StatusExt::FILE_DELETE;
            errorMsg =
                    "failed to delete file: " + file + " in folder '" + folder + "', error: " + ec.message();
        }
    }
    if (!errorMsg.empty()) {
        error(context() + "::" + string(__func__) + " " + errorMsg);
    }
    return {{"req", json::object({{"id", ""},
                                  {"priority", 0},
                                  {"timeout", 0},
                                  {"params", json::object({{"folder", folder}, {"file", file}})}})},
            {"resp", json::object({{"type", ""},
                                   {"expiration_timeout_sec", 0},
                                   {"status", protocol::toString(status)},
                                   {"status_ext", protocol::toString(statusExt)},
                                   {"error", errorMsg},
                                   {"result", json::object()},
