...
private:
    ...
    /**
     * The method is called from the constructor to extract the names of tables according to the initialized
     * value of the batchMode flag. The method is needed to avoid code duplication in the constructor since
     * the names of tables are required for many types of requests.
     *
     * @note In case of a batch request the list of tables must be provided since
     ...
     *
     * @param params the request object received from a client (request-specific parameters)
     * @param batchMode a flag indicating if the request is a batch request
     * @return a collection of tables to be affected by the request
     */
    static std::vector<std::string> _getTables(protocol::RequestParams const& params, bool batchMode) {
        return batchMode ? params.requiredStringVec("tables")
                         : std::vector<std::string>({params.requiredString("table")});
    }
    ...
    http::BinaryEncodingMode _binaryEncodingMode;  ///< The mode for encoding binary data in the result set

    // Input parameters (request-specific, see the constructor for further details)

    std::string _query;                ///< The query to be executed
    std::vector<std::string> _tables;  ///< The collection of tables affected by the request
    std::list<SqlColDef> _columns;     ///< The collection of columns for a table to be created
    ...
    bool _ifExists = true;             ///< A flag to indicate if the operation should be conditional
    bool _ifNotExists = true;          ///< A flag to indicate if the operation should be conditional
