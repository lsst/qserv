...
public:
     ...
     *  INDEX-GET           for extracting and returning a collection of the "director" index data
     *  INDEX-DELETE        for deleting a file in the "director" index
     *  SQL                 for executing various SQL statements against the worker's database
     *  REQUEST-TRACK       for tracking status and retrieving results of the previously submitted request
...
private:
    ...
    /// Process the INDEX-GET request
    nlohmann::json _getIndex();

    /// Process the INDEX-DELETE request
    nlohmann::json _deleteIndex();
