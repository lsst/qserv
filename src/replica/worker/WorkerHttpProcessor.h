...
public:
    ...
    /**
     * Find the request in a collection of the completed requests, and "garbage collect" it to release
     * resources associated with the request. If the request in other state then nothing will
     * happen (no exception thrown, no side effects).
     *
     * @param id an identifier of a request affected by the operation
    ...
    nlohmann::json _submit(replica::Lock const& lock, std::string const& context,
        ...
        try {
            ...
            auto const ptr = REQUEST_TYPE::create(
                    ...
                    [self = shared_from_this()](std::string const& id) { self->stopRequest(id); }, args...);
