...
void WorkerHttpSvc::registerServices(unique_ptr<httplib::Server> const& server) {
    ...
    server->Get("/worker/management/status/:id",
                [self](httplib::Request const& req, httplib::Response& resp) {
                    WorkerHttpSvcMod::process(self->serviceProvider(), self->_processor, self->_workerName,
                                              req, resp, "REQUEST-STATUS");
                });
    server->Put("/worker/management/stop/:id", [self](httplib::Request const& req, httplib::Response& resp) {
        WorkerHttpSvcMod::process(self->serviceProvider(), self->_processor, self->_workerName, req, resp,
                                  "REQUEST-STOP", http::AuthType::REQUIRED);
    });
    server->Delete("/worker/management/dispose",
                   [self](httplib::Request const& req, httplib::Response& resp) {
                       WorkerHttpSvcMod::process(self->serviceProvider(), self->_processor, self->_workerName,
                                                 req, resp, "REQUEST-DISPOSE", http::AuthType::REQUIRED);
                   });
    server->Get("/worker/service", [self](httplib::Request const& req, httplib::Response& resp) {
        WorkerHttpSvcMod::process(self->serviceProvider(), self->_processor, self->_workerName, req, resp,
                                  "SERVICE-STATUS");
    });
