...
void WorkerHttpRequest::checkIfCancelling(replica::Lock const& lock, string const& func) {
    switch (status()) {
        ...
        default:
            throw logic_error(context(func) +
                              "  not allowed while in status: " + protocol::toString(status()));
    }
}
...
void WorkerHttpRequest::init() {
    ...
    if (_requestExpirationIvalSec != 0) {
        ...
        _requestExpirationTimer.async_wait(bind(&WorkerHttpRequest::_expired, shared_from_this(), _1));
...
json WorkerHttpRequest::toJson(bool includeResultIfFinished) const {
    ...
    response["id"] = _id;
    response["type"] = _type;
    response["status"] = protocol::toString(_status, _extendedStatus);
    response["priority"] = _priority;
...
void WorkerHttpRequest::setStatus(replica::Lock const& lock, protocol::Status status,
                                  protocol::StatusExt extendedStatus) {
    LOGS(_log, LOG_LVL_TRACE,
         context(__func__) << "  " << protocol::toString(_status, _extendedStatus) << " -> "
    ...
    switch (status) {
        ...
        case protocol::Status::FAILED:
            ...
            break;
        default:
            throw logic_error(context(__func__) + "  unhandled status: " + protocol::toString(status));
    }
