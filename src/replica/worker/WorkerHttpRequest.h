...
public:
     ...
     * @note The flag includeResultIfFinished=true will be considered to use when the request
     * is successfully finished and the result is available. In this case the result will be included
     * in the response object.
     * @param includeResultIfFinished (optional) flag to include results into the response object
     */
...
protected:
    /**
     * The method is used to get the FQDN of the worker host. The method will retry
     * the DNS request until it gets a response or the request expires. The method
     * will cache the FQDN value for the subsequent calls to avoid unnecessary DNS requests.
     * @param timeoutSec the maximum duration for the DNS request
     * @return the FQDN of the worker host
     * @throws std::runtime_error if the DNS request fails to get a response within the specified timeout.
     */
    static std::string getWorkerFqdn(unsigned int timeoutSec);
    ...
    /// @return the adjusted value of the expiration timeout in seconds.
    unsigned int expirationTimeoutSec() const { return _expirationTimeoutSec; }
...
private:
    ...
    // Request status and performance counters

    std::atomic<protocol::Status> _status{protocol::Status::CREATED};             ///< Primary status
    std::atomic<protocol::StatusExt> _extendedStatus{protocol::StatusExt::NONE};  ///< Secondary status
    std::string _error;              ///< Cached error to be sent to a client (if failed)
    WorkerPerformance _performance;  ///< Atomic performance counters (in any status)

    // Worker FQDB cache management

    static std::string _workerFqdn;        ///< The value is cached after the first successful retrieval.
    static replica::Mutex _mtxWorkerFqdn;  ///< Mutex to guard the access to the cache.
};
