...
HttpAsyncReqApp::HttpAsyncReqApp(int argc, char* argv[])
    ...
    parser().required(
        ...
        "method",
        "The HTTP method. Allowed values: " + ::vector2str(::allowedMethods),
        _method,
        ::allowedMethods
    ).option(
        "header",
...
int HttpAsyncReqApp::runImpl() {
    ...
    auto const ptr = HttpAsyncReq::create(io_service,
        [this, osPtr] (auto const& ptr) {
            this->_dump(ptr, osPtr);
        },
        _method, _url, _data, headers, _maxResponseBodySize, _expirationIvalSec);
