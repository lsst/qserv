...
ExportServer::ExportServer(ServiceProvider::Ptr const& serviceProvider, string const& workerName)
        : _serviceProvider(serviceProvider),
          _workerName(workerName),
          _io_service(),
          _acceptor(_io_service, boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(),
                                                                serviceProvider->config()->get<uint16_t>(
...
void ExportServer::run() {
    ...
    vector<shared_ptr<thread>> threads(
            _serviceProvider->config()->get<size_t>("worker", "num-exporter-processing-threads"));
...
void ExportServer::_beginAccept() {
    ExportServerConnection::Ptr const connection =
            ExportServerConnection::create(_serviceProvider, _workerName, _io_service);
