...
CREATE USER IF NOT EXISTS qsreplica@localhost;
CREATE USER IF NOT EXISTS qsreplica@'%';

CREATE DATABASE IF NOT EXISTS qservReplica;

GRANT ALL ON qservReplica.* TO  qsreplica@localhost;
GRANT ALL ON qservReplica.* TO  qsreplica@'%';

FLUSH PRIVILEGES;


-- Begin copied from src/admin/templates/initdb/sql/repl-db/01_replication_schema.sql
...
-- End copied from src/admin/templates/initdb/sql/repl-db/01_replication_schema.sql

CREATE TABLE IF NOT EXISTS `ReplicaMetadata` (
...
INSERT INTO `ReplicaMetadata` (`metakey`, `value`) VALUES ('version', '0');
