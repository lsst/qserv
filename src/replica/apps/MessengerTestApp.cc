...
MessengerTestApp::MessengerTestApp(int argc, char* argv[])
    ...
    parser().required("worker", "The name of a worker to be used during the testing.", _workerName)
            .option("data", "The data payload to be sent to the worker and expected to be back.", _data)
            .option("proc-time-sec",
                    ...
                    _proccesingTimeSec)
            .option("priority", "The priority level of requests.", _priority)
...
int MessengerTestApp::runImpl() {
    ...
    if (_totalRequests <= 0) {
        throw invalid_argument(context + "the total number of requests must be strictly greater than 0");
    }
    if (_maxActiveRequests > _totalRequests) {
        throw invalid_argument(context + "the number of active requests can't exceed the total number " +
    ...
    for (int i = 0; i < _totalRequests; ++i) {
        ...
        auto const request = EchoRequest::createAndStart(
                ...
                [&](EchoRequest::Ptr request) {
                    ...
                    _logEvent(
                            unique_lock<mutex>(_mtx), request->performance().c_finish_time,
                            "RECV " + request->id() + " " + Request::state2string(request->extendedState()));
                },
                _priority, keepTracking, noParentJobId, _requestExpirationIvalSec);
        _numActive++;

        _logEvent(lock, request->performance().c_create_time, "SEND " + request->id());
    }
...
void MessengerTestApp::_logEvent(unique_lock<mutex> const& lock, uint64_t timeMs, std::string const& event) {
