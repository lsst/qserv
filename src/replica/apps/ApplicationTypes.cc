...
Command& Command::flag(string const& name, string const& description, bool& var) {
    bool const reverse = false;
    _flags.emplace(name, make_unique<FlagParser>(name, description, var, reverse));
    return *this;
}

Command& Command::reversedFlag(string const& name, string const& description, bool& var) {
    bool const reverse = true;
    _flags.emplace(name, make_unique<FlagParser>(name, description, var, reverse));
    return *this;
}

CommandsSet::CommandsSet(vector<string> const& commandNames, string& var) : _var(var) {
...
Command& CommandsSet::command(string const& name) {
    ...
    if (itr == _commands.end()) {
        throw range_error("CommandsSet::" + string(__func__) + "  unknown command name: '" + name + "'");
    }
    return *(itr->second);
}

Parser::Parser(int argc, const char* const argv[], string const& description)
        : _argc(argc), _argv(argv), _description(description), _code(Status::UNDEFINED) {}
...
Parser& Parser::commands(string const& name, vector<string> const& commandNames, string& var) {
...
Command& Parser::command(string const& name) {
    if (_commands == nullptr) {
        throw logic_error("Parser::" + string(__func__) + "  the parser is not configured in this way");
    }
    return _commands->command(name);
}

Parser& Parser::flag(string const& name, string const& description, bool& var) {
    _verifyArgument(name);
    bool const reverse = false;
    _flags.emplace(make_pair(name, make_unique<FlagParser>(name, description, var, reverse)));
    return *this;
}

Parser& Parser::reversedFlag(string const& name, string const& description, bool& var) {
    _verifyArgument(name);
    bool const reverse = true;
    _flags.emplace(make_pair(name, make_unique<FlagParser>(name, description, var, reverse)));
    return *this;
...
Parser::Status Parser::parse() {
    ...
    try {
        ...
        size_t const maxNumParameters =
                _required.size() + _optional.size() +
                (commandMode ? _commands->_commands[commandName]->_required.size() +
                                       _commands->_commands[commandName]->_optional.size()
                             : 0);

        if (inNumParameters > maxNumParameters) {
            throw ParserError(ERR_LOC, "too many positional parameters " + to_string(inNumParameters) +
                                               ", expected no more than " + to_string(maxNumParameters));
        }

        size_t const minNumParameters =
                _required.size() + (commandMode ? _commands->_commands[commandName]->_required.size() : 0);

        if (inNumParameters < minNumParameters) {
            throw ParserError(ERR_LOC, "insufficient number " + to_string(inNumParameters) +
                                               " of positional parameters, expected at least " +
                                               to_string(minNumParameters));
        }
...
bool Parser::_parseOption(map<string, unique_ptr<ArgumentParser>>& options, string const& name,
                          string const& value) {
...
bool Parser::_parseFlag(map<string, unique_ptr<ArgumentParser>>& flags, string const& name) {
...
void Parser::_parseParameters(vector<unique_ptr<ArgumentParser>>& out, vector<string>::const_iterator& inItr,
...
void Parser::_verifyArgument(string const& name) {
    if (name.empty()) {
        throw invalid_argument("Parser::" + string(__func__) +
                               "  empty string passed where argument name "
                               "was expected");
    }
    if (name == "help") {
...
string Parser::_wrap(string const& str, string const& indent, size_t width) {
    ostringstream os;
    size_t lineLength = 0;
...
string Parser::serializeArguments() const {
    if (Status::SUCCESS != _code) {
        throw logic_error("Parser::" + string(__func__) +
