...
ChunkedTable::ChunkedTable(string const& name) {
    ...
    if (regex_search(name, match, reFullOverlapTable) and match.size() == 3) {
        ...
        throw invalid_argument("ChunkedTable::" + string(__func__) + "failed to parse the table name '" +
                               name +
...
void ChunkedTable::_assertValid() const {
    if (_baseName.empty()) {
        throw invalid_argument("ChunkedTable::" + string(__func__) + "  object is not in the valid state");
    }
}
