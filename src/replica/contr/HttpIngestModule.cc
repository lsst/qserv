...
json HttpIngestModule::_addTable() {
    ...
    for (auto&& thisTableName : tables) {
        auto const job = SqlCreateTableJob::create(database.name, thisTableName, engine, _partitionByColumn,
                                                   table.columns, table.charsetName, table.collationName,
...
void HttpIngestModule::_createMissingChunkTables(DatabaseInfo const& database, bool allWorkers) const {
    ...
    for (auto&& tableName : database.partitionedTables()) {
        ...
        auto const job = SqlCreateTablesJob::create(
                database.name, table.name, engine, _partitionByColumn, table.columns, table.charsetName,
                table.collationName, allWorkers, controller(), noParentJobId, nullptr,
