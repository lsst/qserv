...
IngestSvc::IngestSvc(ServiceProvider::Ptr const& serviceProvider, string const& workerName)
...
void IngestSvc::run() {
    ...
    vector<shared_ptr<thread>> threads(
            _serviceProvider->config()->get<size_t>("worker", "num-loader-processing-threads"));
...
void IngestSvc::_beginAccept() {
    IngestSvcConn::Ptr const connection = IngestSvcConn::create(_serviceProvider, _workerName, _io_service);

    _acceptor.async_accept(connection->socket(),
                           bind(&IngestSvc::_handleAccept, shared_from_this(), connection, _1));
}

void IngestSvc::_handleAccept(IngestSvcConn::Ptr const& connection, boost::system::error_code const& ec) {
    if (ec.value() == 0) {
