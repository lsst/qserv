...
bool QueryRunner::runQuery() {
    util::InstanceCount ic("QI=" + to_string(_task->getQueryId()) + "_rq_LDB");  // LockupDB
...
bool QueryRunner::_dispatchChannel() {
    ...
    try {
        ...
        if (!_cancelled && !_task->getSendChannel()->isDead()) {
            ...
            util::InstanceCount ica("QI=" + to_string(_task->getQueryId()) + "_rqa_LDB");  // LockupDB
    ...
    util::InstanceCount icc("QI=" + to_string(_task->getQueryId()) + "_rqc_LDB");  // LockupDB
    if (!readRowsOk) {
