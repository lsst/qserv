...
struct QuerySql::Batch {
    ...
    Batch(std::string const& name_, QuerySql::StringDeque const& sequence_, int batchSize_ = 10)
            : name(name_), batchSize(batchSize_), pos(0) {
        for (QuerySql::StringDeque::const_iterator i = sequence_.begin(); i != sequence_.end(); ++i) {
    ...
    bool isDone() const { return sequence.empty() || (static_cast<size_t>(pos) >= sequence.size()); }
    std::string current() const {
        ...
        if (sequence.size() < static_cast<size_t>(pos + batchSize)) {
            std::copy(begin, sequence.end(), std::ostream_iterator<std::string>(os, ";\n"));
        } else {
            std::copy(begin, begin + batchSize, std::ostream_iterator<std::string>(os, ";\n"));
        }
        return os.str();
    }
