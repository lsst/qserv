...
**Important**: The ``MultipartEncoder`` class from the ``requests_toolbelt`` package is used for both formatting
...
The services will locate and evaluate the specified request to see if it's eligible for retrying. And if it's then
...
``retry_allowed`` : *number*
  ...
  **Important**: The workflow should be always analyze a value of this attribute to decide if the contribution should be retried.
  ...
  the another transaction to ingest all contributions attempted in a scope of the aborted one.
