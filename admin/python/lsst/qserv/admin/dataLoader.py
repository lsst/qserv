...
class DataLoader(object):
    ...
    def __init__(self, configFiles, mysqlConn, workerMysqlConn={}, chunksDir="./loader_chunks",
        ...
        @param workerMysqlConn: Dictionary mapping worker host name to corresponding
        ...
        self.workerMysqlConn = workerMysqlConn.copy()
    ...
    def _run(self, database, table, schema, data):
        ...
        # make chunk mapper
        self.chunkMap = ChunkMapping(self.workerMysqlConn.keys(), database, table, self.css)
    ...
    def _connections(self, useCzar, useWorkers):
        ...
        if useCzar:
            res += [("czar", self.mysql)]
        if useWorkers:
            for worker, conn in self.workerMysqlConn.items():
    ...
    def _deleteTable(self, database, table):
        ...
        for name, conn in self._connections(True, True):
    ...
    def _createDummyChunk(self, database, table):
        ...
        connections = self._connections(False, True)
        if not connections:
            connections = self._connections(True, False)
    ...
    def _makeIndexMultiNode(self, database, table, metaTable, idxCol):
        ...
        for chunk in self.chunks:
            ...
            conn = self.workerMysqlConn[wname]
            wCursor = conn.cursor()
