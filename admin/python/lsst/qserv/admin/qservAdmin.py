...
class QservAdmin(object):
    ...
    def _addPacked(self, origPath, valueDict):
        ...
        return self._kvI.create(path=jsonPath, value=json.dumps(valueDict))
    ...
    def createDb(self, dbName, options):
        ...
        with self._getDbLock(dbName):
            try:
                ...
                partOptions = dict((k,v) for k,v in options
                                   if k in set( ["nStripes", "nSubStripes",
                                                 "overlap", "uuid"]))
                self.addPackedEntry(ptP, partOptions)
                # old
                ...
                self._addPackedEntry(dbP, dbOpts)
    ...
    def _normalizeTableOpts(tbOpts, matchOpts, partitionOpts):
        """Apply defaults and fixups to table options."""
        def check(d, possible):
            for k in possible["table"] :
                if k not in d:
    ...
    def _createDbLockSection(self, dbP):
        ...
        self._kvI.addPacked(dbP + "LOCK.json", lockOpts)
